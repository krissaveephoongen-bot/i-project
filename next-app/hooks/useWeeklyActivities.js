...
export const useWeeklyActivities = (selectedDate = new Date()) => {
  ...
  const fetchWeeklyActivities = async (date = selectedDate) => {
    ...
    try {
            ...
            if (!employeeMap.has(entry.userId)) {
              employeeMap.set(entry.userId, {
                ...
                days: Array(7).fill(null).map((_, index) => ({
                  day: format(new Date(weekStart.getTime() + index * 24 * 60 * 60 * 1000), 'EEE dd MMM'),
                  activities: []
                }))
              });
            }
            
            const dayIndex = Math.floor((new Date(day.date) - weekStart) / (24 * 60 * 60 * 1000));
      ...
      const transformedActivities = Array.from(employeeMap.values())
