...
export class DashboardCache {
  ...
  static async setStats(stats: DashboardStats): Promise<void> {
    ...
    await redis.set(this.STATS_KEY, JSON.stringify(statsWithTimestamp), { EX: this.STATS_TTL });
  }
  ...
  static async getActivityFeed(projectId?: string, ttl: number = 600): Promise<any[] | null> {
  ...
  static async setActivityFeed(activities: any[], projectId?: string, ttl: number = 600): Promise<void> {
    const key = `activity_feed:${projectId || 'all'}`;
    await redis.set(key, JSON.stringify(activities), { EX: ttl });
  }

  // Cache user notifications
  static async getUserNotifications(userId: string, ttl: number = 300): Promise<any[] | null> {
  ...
  static async setUserNotifications(userId: string, notifications: any[], ttl: number = 300): Promise<void> {
    const key = `notifications:${userId}`;
    await redis.set(key, JSON.stringify(notifications), { EX: ttl });
  }

  // Cache frequently accessed data
  static async cacheData(key: string, data: any, ttl: number = 300): Promise<void> {
  ...
  static async setMultiple(keys: string[], data: any[], ttl: number = 300): Promise<void> {
