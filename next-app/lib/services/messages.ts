...
export class MessageService {
  ...
  static async fetchThreadById(threadId: string): Promise<ThreadWithDetails | null> {
    const { data, error } = await supabase
      .from('threads')
      .select(`
        *,
  ...
  // Add user to thread
  static async addThreadMember(threadId: string, userId: string): Promise<void> {
  ...
  static async removeThreadMember(threadId: string, userId: string): Promise<void> {
    const { error } = await supabase
      .from('thread_members')
      .delete()
  ...
  static async fetchThreadMessages(threadId: string): Promise<MessageWithSender[]> {
    const { data, error } = await supabase
      .from('messages')
      .select(`
        *,
  ...
  static subscribeToThreadMessages(threadId: string, callback: (payload: any) => void) {
    return supabase
