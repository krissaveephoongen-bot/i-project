...
import { Project, ProjectInsert, ProjectUpdate, ProjectWithManager } from '../../types/database.types'
...
export class ProjectService {
  ...
  static async fetchProjectById(id: string): Promise<ProjectWithManager | null> {
    const { data, error } = await supabase
      .from('projects')
      .select(`
        *,
  ...
  static async fetchProjectsByManager(managerId: string): Promise<ProjectWithManager[]> {
    const { data, error } = await supabase
      .from('projects')
      .select(`
        *,
  ...
  static async fetchProjectsByClient(clientId: string): Promise<ProjectWithManager[]> {
    const { data, error } = await supabase
      .from('projects')
      .select(`
        *,
  ...
  static async updateProjectStatus(id: string, status: string): Promise<Project> {
    return this.updateProject(id, { status })
  }

  // Update project budget
  static async updateProjectBudget(id: string, budget: number): Promise<Project> {
