...
export class UserService {
  ...
  static async createUser(user: UserInsert & { password: string }): Promise<User> {
    // Hash the password
  ...
  static async updateUserDepartment(id: string, department: string): Promise<User> {
  ...
  static async getUserStats(userId: string): Promise<any> {
    ...
    const stats = {
      ...
      activeProjects: profile.managed_projects?.filter((p: any) => p.status === 'in_progress').length || 0,
      totalTasks: profile.assigned_tasks?.length || 0,
      completedTasks: profile.assigned_tasks?.filter((t: any) => t.status === 'done').length || 0,
      totalHours: profile.time_entries?.reduce((sum: number, entry: any) => sum + (entry.hours || 0), 0) || 0,
      totalEarnings: (profile.time_entries?.reduce((sum: number, entry: any) => sum + (entry.hours || 0), 0) || 0) * (profile.hourlyRate || 0)
    }
