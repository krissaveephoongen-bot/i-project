...
export class TimesheetService {
  ...
  static async fetchTimesheetsByUser(
    userId: string,
  ...
  static async fetchTimesheetsByProject(
    projectId: string,
  ): Promise<TimesheetWithDetails[]> {
    const { data, error } = await supabase
      .from("time_entries")
  ...
  static async upsertTimesheetWeek(
    week: TimesheetWeekInsert,
  ...
  static async createTimesheetEntry(
    entry: TimesheetEntryInsert,
  ): Promise<TimesheetEntry> {
    const { data, error } = await supabase
      .from("timesheet_entries")
      .insert(entry)
