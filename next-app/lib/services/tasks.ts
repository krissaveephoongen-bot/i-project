...
import { Task, TaskInsert, TaskUpdate, TaskWithProject } from '../../types/database.types'
...
export class TaskService {
  ...
  static async fetchTasksByProject(projectId: string): Promise<TaskWithProject[]> {
    const { data, error } = await supabase
      .from('tasks')
      .select(`
        *,
  ...
  static async fetchTasksByAssignee(assigneeId: string): Promise<TaskWithProject[]> {
    const { data, error } = await supabase
      .from('tasks')
      .select(`
        *,
  ...
  static async fetchTasksByPriority(priority: string): Promise<TaskWithProject[]> {
    const { data, error } = await supabase
      .from('tasks')
      .select(`
        *,
