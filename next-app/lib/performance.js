  ...
  generateRecommendations() {
    ...
    if (this.metrics.pageLoad.loadComplete > 3000) {
      recommendations.push({
        ...
        severity: "high",
        message:
          "Page load time is slow. Consider optimizing images, reducing JavaScript bundle size, or implementing lazy loading.",
    ...
    if (this.metrics.vitals.lcp?.value > 2500) {
      recommendations.push({
        ...
        severity: "high",
        message:
          "Largest Contentful Paint (LCP) is slow. Optimize above-the-fold content and reduce server response time.",
    ...
    if (this.metrics.vitals.fid?.value > 100) {
      recommendations.push({
        ...
        severity: "medium",
        message:
          "First Input Delay (FID) is high. Reduce JavaScript execution time and break up long tasks.",
    ...
    if (this.metrics.vitals.cls?.value > 0.1) {
      recommendations.push({
        ...
        severity: "high",
        message:
          "Cumulative Layout Shift (CLS) is high. Avoid layout shifts by specifying dimensions for images and videos.",
