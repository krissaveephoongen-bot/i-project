...
export interface Project {
  ...
  payment_schedule?: { installment: number; percentage: number; amount?: number }[];
}
...
export const recalculateProjectData = async (project: any) => {
  ...
  if (project.start_date && project.end_date) {
    ...
    elapsedTime = Math.ceil((today.getTime() - start.getTime()) / (1000 * 60 * 60 * 24)) + 1;
    
    remainingTime = Math.ceil((end.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
  }
...
export async function createProject(newProject: Omit<Project, 'id' | 'created_at' | 'updated_at' | 'manager_name' | 'task_count' | 'total_duration' | 'elapsed_time' | 'remaining_time'>): Promise<Project> {
  const { data, error } = await supabase
    .from('projects')
    .insert({
      ...newProject,
...
export async function updateProject(id: string, updatedFields: Partial<Omit<Project, 'id' | 'created_at' | 'updated_at' | 'manager_name' | 'task_count' | 'total_duration' | 'elapsed_time' | 'remaining_time'>>): Promise<Project> {
  const { data, error } = await supabase
    .from('projects')
    .update({
      ...updatedFields,
