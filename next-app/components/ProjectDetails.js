  ...
  try {
    ...
    const loadProjectData = async () => {
      try {
        ...
        setTasks(
          taskResult.items?.filter(
            (t) => t.objectData.ProjectId === project.id,
          ) || [],
        );
        setExpenses(
          expenseResult.items?.filter(
            (e) => e.objectData.ProjectId === project.id,
          ) || [],
        );
        setWorkLogs(
          workLogResult.items?.filter(
            (w) => w.objectData.ProjectId === project.id,
          ) || [],
        );
      } catch (error) {
    ...
    const totalExpenses = expenses.reduce(
      (sum, e) => sum + (e.objectData?.Amount || 0),
      0,
    );
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-sm font-semibold ${
                        Math.abs(
                          tasks.reduce(
                            (sum, t) => sum + (t.objectData.Weight || 0),
                            0,
                          ) - 100,
                        ) < 0.01
                          ? "text-green-600"
                    ...
                    >
                      {tasks
                        .reduce((sum, t) => sum + (t.objectData.Weight || 0), 0)
                        .toFixed(1)}
                      %
                      {Math.abs(
                        tasks.reduce(
                  ...
                  {tasks.map((task) => (
                    <div key={task.objectId} className="card">
                      <div className="flex items-start justify-between">
                        ...
                        <span
                          className={`status-badge status-${task.objectData.Status}`}
                        >
                ...
                {expenses.map((expense) => (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-sm text-[var(--text-secondary)]">
                        {expense.objectData.Description}
                      </div>
                    </div>
                ...
                {workLogs.map((log) => (
                  <div key={log.objectId} className="card">
                    <div className="flex items-center justify-between mb-2">
                      <div className="text-sm text-[var(--text-secondary)]">
                        {new Date(log.objectData.Date).toLocaleDateString(
                          "th-TH",
                        )}
                      </div>
