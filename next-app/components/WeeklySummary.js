...
function WeeklySummary() {
  ...
  try {
    ...
    const handleExportData = () => {
      const data = {
        สรุปสัปดาห์: selectedRange === 'current' ? 'สัปดาห์ปัจจุบัน' : selectedRange === 'last' ? 'สัปดาห์ที่แล้ว' : 'ช่วงเวลาที่เลือก',
    ...
    return (
      <div className="card" data-name="weekly-summary" data-file="components/WeeklySummary.js">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={refreshSummary} className="p-1 hover:bg-gray-100 rounded-lg transition-colors">
          ...
          <div className="bg-gray-50 rounded-lg p-4 mb-4 space-y-3">
            ...
            <div className="grid grid-cols-1 gap-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{summary.tasksCompleted > 0 ? 'ดีเยี่ยม' : 'ปกติ'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">เฉลี่ยชั่วโมง/วัน:</span>
                <span className="font-medium">{(summary.hoursWorked / 7).toFixed(1)} ชม.</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">อัตราส่วนการประชุม:</span>
                <span className="font-medium">{summary.meetingsAttended > 0 ? `${((summary.meetingsAttended / 7) * 100).toFixed(0)}%` : '0%'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">สถานะโดยรวม:</span>
                <span className={`font-medium ${summary.hoursWorked >= 35 ? 'text-green-600' : 'text-orange-600'}`}>
