      ...
      if (tasks.items) {
        tasks.items.forEach(task => {
          if (task.objectData?.dueDate) {
            ...
            if (diff <= 3 && diff >= 0 && task.objectData.status !== 'completed') {
              notifs.push({
                id: task.objectId,
                type: 'task',
              ...
              notifications.map(notif => (
                <div key={notif.id} className="p-4 border-b border-gray-100 hover:bg-gray-50 cursor-pointer">
                  <div className="flex items-start gap-3">
                    <div className={`icon-${notif.type === 'task' ? 'alert-circle' : 'check-circle'} text-lg ${notif.type === 'task' ? 'text-orange-500' : 'text-blue-500'}`}></div>
