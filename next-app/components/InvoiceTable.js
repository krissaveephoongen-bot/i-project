function InvoiceTable({ projectId }) {
  ...
  const loadInvoices = async () => {
    try {
      ...
      const result = await trickleListObjects("invoice", 100, true).catch(
        () => ({ items: [] }),
      );
              ...
              <tr key={inv.objectId} className="border-b hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <div className="text-xs space-y-1">
                    ...
                    <div>
                      Plan Received: {inv.objectData.PlanReceivedDate || "-"}
                    </div>
...
function InvoiceForm({ projectId, onSave }) {
  ...
  return (
    <form
      ...
      className="bg-blue-50 p-4 rounded-lg mb-4 grid grid-cols-2 gap-3"
    >
      <input
        type="number"
        placeholder="งวดที่"
      ...
      <button
        type="submit"
        className="col-span-2 px-4 py-2 bg-[var(--primary-color)] text-white rounded"
      >
        บันทึก
      </button>
    </form>
