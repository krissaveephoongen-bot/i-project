...
const VendorSystemTestProps: React.FC<VendorSystemTestProps> = ({
  ...
  const runTests = async () => {
        ...
        tests: [
          {
            ...
            message: response.ok
              ? "ดึงข้อมูล vendor สำเร็จ"
              : "ไม่สามารถดึงข้อมูล vendor",
        ...
        tests: [
          {
            ...
            status: response.ok ? "success" : "error",
            message: response.ok
              ? "ดึงข้อมูลสัญญาสำเร็จ"
              : "ไม่สามารถดึงข้อมูลสัญญา",
            details: response.ok ? undefined : data.error,
          },
          {
        ...
        tests: [
          {
            ...
            status: response.ok ? "success" : "error",
            message: response.ok
              ? "ดึงข้อมูลรายการค่าใช้จ่ายสำเร็จ"
              : "ไม่สามารถดึงข้อมูลรายการค่าใช้จ่าย",
            details: response.ok ? undefined : data.error,
          },
          {
        ...
        tests: [
          {
            ...
            status: response.ok ? "success" : "error",
            message: response.ok
              ? "ดึงข้อมูลการประเมิน KPI สำเร็จ"
              : "ไม่สามารถดึงข้อมูลการประเมิน KPI",
            details: response.ok ? undefined : data.error,
          },
          {
    ...
    try {
      ...
      results.push({
        ...
        successRate:
          (uiTests.filter((t) => t.status === "success").length /
            uiTests.length) *
          100,
      });
    } catch (error) {
  ...
  const getOverallStatus = (results: TestResult[]) => {
    ...
    if (overallSuccessRate >= 90) {
      return {
        status: "success",
        message: "ระบบทำงานได้ดีเยี่ยม",
        color: "text-green-600",
      };
    } else if (overallSuccessRate >= 70) {
      return {
        status: "warning",
        message: "ระบบทำงานได้ดี แต่มีบางส่วนที่ต้องแก้ไข",
        color: "text-yellow-600",
      };
    } else {
      return {
        status: "error",
        message: "ระบบมีปัญหาที่ต้องแก้ไขก่อนทำงานได้",
        color: "text-red-600",
      };
    }
  };
              ...
              <Card key={index}>
                <CardHeader>
                  ...
                  <div className="flex items-center mt-2">
                    ...
                    <Badge
                      variant={
                        result.successRate >= 80 ? "default" : "destructive"
                      }
                      className="ml-auto"
                    >
