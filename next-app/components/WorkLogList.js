  ...
  try {
    ...
    const loadWorkLogs = async () => {
      try {
        ...
        const result = await trickleListObjects("worklog", 30, true).catch(
          (err) => {
    ...
    return (
      <div
        className="space-y-4"
        data-name="worklog-list"
        data-file="components/WorkLogList.js"
      >
        <div className="card">
                ...
                {workLogs.map((log) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">
                      <span
                        className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                          log.objectData.WorkType === "project"
                            ? "bg-blue-100 text-blue-800"
                            : log.objectData.WorkType === "office"
                    ...
                    <td className="py-3 px-4 font-medium text-[var(--primary-color)]">
                      {calculateManday(
                        log.objectData.StartTime,
                        log.objectData.EndTime,
                      )}
                    </td>
                    <td className="py-3 px-4">
                      <span
                        className={`status-badge ${getStatusColor(log.objectData.Status)}`}
                      >
