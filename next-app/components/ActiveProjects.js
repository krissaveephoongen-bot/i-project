function ActiveProjects({ projects }) {
  try {
    ...
    return (
      <div
        className="bg-background-base rounded-lg border border-neutral-200 p-6 shadow-sm"
        data-name="active-projects"
        data-file="components/ActiveProjects.js"
      >
        ...
        <div className="space-y-4">
          {activeProjects.map((project) => (
            <div
              key={project.id}
              className="p-4 bg-background-light rounded-lg border border-neutral-200 shadow-xs hover:shadow-sm transition-shadow"
            >
