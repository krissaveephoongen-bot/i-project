  ...
  try {
    ...
    return (
      <div
        className="space-y-3"
        data-name="task-list"
        data-file="components/TaskList.js"
      >
        {tasks.map((task) => (
          <div
            key={task.id}
            className={`${detailed ? "bg-background-base" : "bg-background-light"} border border-neutral-200 rounded-lg p-4 shadow-xs hover:shadow-sm transition-shadow`}
          >
            <div className="flex items-start justify-between">
              <div className="flex items-start space-x-3 flex-1">
                ...
                <div className="flex-1">
                  <h4
                    className={`font-medium ${task.status === "completed" ? "line-through text-neutral-400" : "text-neutral-900"}`}
                  >
                    {task.title}
                  </h4>
                  {detailed && (
