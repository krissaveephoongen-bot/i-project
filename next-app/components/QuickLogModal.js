  ...
  try {
    ...
    const loadProjects = async () => {
      try {
        ...
        const result = await trickleListObjects("project", 20, true).catch(
          () => ({ items: [] }),
        );
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              <input
                ...
                className="w-24 px-4 py-3 border rounded-xl text-center font-bold"
              />
            </div>
            <button
              type="submit"
              className="w-full py-3 bg-[var(--primary-color)] text-white rounded-xl font-semibold"
            >
              Save Log
            </button>
          </form>
