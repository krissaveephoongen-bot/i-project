...
const WeeklyActivities = () => {
  ...
  const { activities, loading, error, refreshActivities } = useWeeklyActivities(selectedWeek);
  ...
  return (
    <Card>
      ...
      <div className="flex items-center gap-4 mb-6">
        <label className="text-sm font-medium text-gray-700">สัปดาห์ที่เลือก:</label>
        <div className="flex items-center gap-2">
          ...
          <div className="px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg min-w-[120px] text-center">
            <span className="text-sm font-medium text-gray-900">{getWeekDisplay()}</span>
          </div>
          ...
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              {mockActivities[0]?.days.map((day, index) => (
                <th key={index} className="px-4 py-3 text-center text-xs font-medium text-gray-700 uppercase tracking-wider border border-gray-200 min-w-[120px]">
                  {day.day}
                </th>
              ))}
            </tr>
            ...
            {filteredActivities.map((employee, empIndex) => (
              <tr key={empIndex} className={empIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                ...
                {employee.days.map((day, dayIndex) => (
                  <td key={dayIndex} className="px-4 py-3 text-sm text-gray-600 border border-gray-200 align-top">
                    <div className="space-y-1 min-h-[60px]">
                      {day.activities[0] === '-' ? (
                        <span className="text-gray-400">-</span>
                      ) : (
                        day.activities.map((activity, actIndex) => (
                          <div key={actIndex} className="text-xs leading-relaxed">
                            {activity}
                          </div>
                        ))
                      )}
      ...
      {filteredActivities.length === 0 && (
        <div className="text-center py-8">
          ...
          <p className="text-gray-600">ไม่พบข้อมูลกิจกรรมสำหรับพนักงานที่ค้นหา</p>
        </div>
      )}
      </CardContent>
    </Card>
