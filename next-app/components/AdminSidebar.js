...
function AdminSidebar({ isOpen }) {
  ...
  return (
    <div
      className={`fixed left-0 top-0 h-full bg-[#1a365d] text-white transition-all duration-300 flex flex-col ${isOpen ? "w-64" : "w-0 overflow-hidden"}`}
    >
      <div className="p-6 border-b border-white/10">
        <div className="text-2xl font-bold animate-[fadeIn_0.5s_ease-out]">
          Transit Subsidy
        </div>
      ...
      <nav className="p-4 flex-1">
        {menuItems.map((item, i) => (
          <div key={i} className="has-tooltip relative">
            <a
              href={item.path}
              className={`flex items-center px-4 py-3 mb-2 rounded-xl transition-all duration-300 ${currentPage === item.id ? "bg-[#00C7BE] shadow-lg" : "hover:bg-white/10 hover:translate-x-1"}`}
            >
              <div className={`icon-${item.icon} text-xl mr-3`}></div>
              <span>{item.label}</span>
            </a>
            {!isOpen && (
              <span className="tooltip bg-gray-800 text-white text-xs rounded py-1 px-2 left-16 top-1/2 -translate-y-1/2">
                {item.label}
              </span>
            )}
          </div>
