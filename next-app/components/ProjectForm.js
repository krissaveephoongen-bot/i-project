...
const ProjectForm = ({ project, onSave, onCancel, isOpen }) => {
    ...
    useEffect(() => {
        ...
        if (project) {
            setFormData({
                ...
                payment_percentage: project.payment_percentage || 100.00,
    ...
    const handleChange = (e) => {
        ...
        setFormData(prev => ({
            ...prev,
            [name]: name === 'progress' || name === 'budget' || name === 'spent'
                ? Number(value)
                : value
        }));
    };
    ...
    const addPaymentScheduleItem = () => {
        ...
        setFormData(prev => ({
            ...prev,
            payment_schedule: [...prev.payment_schedule, { installment: newInstallment, percentage: 0 }]
        }));
    };
    ...
    const removePaymentScheduleItem = (index) => {
        ...
        const renumberedSchedule = newSchedule.map((item, i) => ({ ...item, installment: i + 1 }));
            ...
            <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto m-4">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="Fixed Price">Fixed Price / Lump Sum (สัญญาจ้างเหมาแบบราคาคงที่)</option>
                                <option value="Time & Materials">Time & Materials (T&M) (สัญญาจ้างตามระยะเวลาและวัสดุ)</option>
                                <option value="Cost Plus">Cost Plus / Cost Reimbursable (สัญญาจ้างแบบต้นทุนบวกกำไร)</option>
                                <option value="Unit Price">Unit Price / Unit Rate (สัญญาจ้างแบบราคาต่อหน่วย)</option>
                                <option value="Retainer">Retainer / Service Agreement (สัญญาจ้างบริการรายงวด/รายเดือน)</option>
                                <option value="Turnkey">Turnkey (สัญญาจ้างเหมาเบ็ดเสร็จ)</option>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                min="0"
                                step="0.01"
                                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.budget ? 'border-red-500' : 'border-gray-300'
                                    }`}
                            />
                            {errors.budget && (
                        ...
                        <div>
                            ...
                            <input
                                ...
                                min="0"
                                step="0.01"
                                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.spent ? 'border-red-500' : 'border-gray-300'
                                    }`}
                            />
                            {errors.spent && (
                                <p className="text-sm text-red-500 flex items-center gap-1 mt-1">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={handleChange}
                                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.end_date ? 'border-red-500' : 'border-gray-300'
                                    }`}
                            />
                            {errors.end_date && (
                                <p className="text-sm text-red-500 flex items-center gap-1 mt-1">
                        ...
                        {formData.payment_schedule.length > 0 ? (
                            <div className="border border-gray-200 rounded-lg p-3 space-y-2">
                                    ...
                                    <div key={index} className="grid grid-cols-12 gap-2 items-center">
                                        <div className="col-span-2">
                                            <input
                                                ...
                                                onChange={(e) => handlePaymentScheduleChange(index, 'installment', e.target.value)}
                                                min="1"
                                        ...
                                        <div className="col-span-4">
                                            <input
                                                ...
                                                onChange={(e) => handlePaymentScheduleChange(index, 'percentage', e.target.value)}
                                                min="0"
                                                max="100"
                                                step="0.01"
                                                className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:ring-1 focus:ring-blue-500"
                                        ...
                                        <div className="col-span-4 text-sm text-gray-600">
                                            {formData.budget ? `฿${((formData.budget * schedule.percentage) / 100).toLocaleString()}` : '-'}
                                        </div>
                                        <div className="col-span-2">
                                            <button
                                                type="button"
                                ...
                                <div className="pt-2 border-t text-xs text-gray-600">
                                    รวม: {formData.payment_schedule.reduce((sum, s) => sum + s.percentage, 0).toFixed(2)}%
                                    {formData.payment_percentage && (
                                        <span className={`ml-2 ${formData.payment_schedule.reduce((sum, s) => sum + s.percentage, 0).toFixed(2) !== formData.payment_percentage.toFixed(2) ? 'text-orange-600 font-medium' : ''}`}>
                            ...
                            <div className="border border-gray-200 rounded-lg p-4 text-center text-gray-500 text-sm">
                                ยังไม่มีตารางงวดการชำระเงิน คลิก "แบ่งเท่ากัน" หรือ "+ เพิ่มงวด" เพื่อเริ่มต้น
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex justify-end gap-3 pt-4">
                        ...
                        >
                            {loading ? 'กำลังบันทึก...' : (project?.id ? 'อัปเดต' : 'เพิ่มโครงการ')}
                        </button>
                    </div>
                </form>
            </div>
