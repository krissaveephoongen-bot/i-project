function DeliveryScheduleTable({ project }) {
  ...
  const loadSchedules = async () => {
    try {
      const result = await trickleListObjects(
        `delivery:${project.objectId}`,
        100,
        true,
      );
  ...
  const calculateTotals = () => {
    const totalAmount = schedules.reduce(
      (sum, s) => sum + (parseFloat(s.objectData.Amount) || 0),
      0,
    );
    const receivedAmount = schedules.reduce((sum, s) => {
      return s.objectData.ReceiptDate
        ? sum + (parseFloat(s.objectData.Amount) || 0)
        : sum;
    }, 0);
    return {
      totalAmount,
      receivedAmount,
      remaining: totalAmount - receivedAmount,
      percentage:
        totalAmount > 0 ? ((receivedAmount / totalAmount) * 100).toFixed(1) : 0,
    };
  };
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-purple-300">
              ...
              <th
                colSpan="5"
                className="border border-gray-300 px-3 py-2 text-center bg-purple-200"
              >
                การชำระเงิน
              </th>
            </tr>
                ...
                >
                  ...
                  <td className="border border-gray-300 px-3 py-2 text-center">
                    {isEditing ? (
                      <input
                        ...
                        value={data.PlanDate || ""}
                        onChange={(e) =>
                          setEditForm({ ...editForm, PlanDate: e.target.value })
                        }
                        className="w-full px-2 py-1 border rounded"
                      />
                    ) : (
                  ...
                  <td className="border border-gray-300 px-3 py-2 text-center">
                    {isEditing ? (
                      <input
                        type="date"
                        value={data.PlanReceived || ""}
                        onChange={(e) =>
                          setEditForm({
                            ...editForm,
                            PlanReceived: e.target.value,
                          })
                        }
                        className="w-full px-2 py-1 border rounded"
                      />
            ...
            <tr>
              ...
              <td colSpan="5" className="border border-gray-300 px-3 py-3">
                <div className="flex items-center gap-4 text-sm">
                  <span>
                    รับชำระแล้ว: ฿{totals.receivedAmount.toLocaleString()} (
                    {totals.percentage}%)
                  </span>
