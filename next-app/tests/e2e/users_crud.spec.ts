...
test.describe('Users CRUD', () => {
   test('Create, Edit, Delete user', async ({ page }) => {
    ...
    await page.addInitScript((id) => {
     ...
     const firstRowNameInput = page.locator('tbody tr').first().locator('input').nth(0);
     await firstRowNameInput.fill(`${name} Edited`);
     await firstRowNameInput.blur();
 
     await expect(page.getByText(`${name} Edited`)).toBeVisible({ timeout: 5000 });
 
     page.once('dialog', dialog => dialog.accept());
     await page.locator('tbody tr').first().getByRole('button', { name: 'ลบ' }).click();
 
     await expect(page.getByText(`${name} Edited`)).toBeHidden({ timeout: 5000 });
   });
