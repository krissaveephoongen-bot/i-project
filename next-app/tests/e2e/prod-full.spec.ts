...
test.describe("Production Full Navigation and Data Validation", () => {
  test("navigate all pages, tabs, components, and record report", async ({
    page,
  }) => {
    ...
    const roleButton = page.getByRole("button", {
      name: /เข้าสู่ระบบ|login|sign in/i,
    });
    if (
      await roleButton
        .first()
    ...
    for (const route of staticRoutes) {
      ...
      try {
        ...
        const hasMain = await page
          .locator('main, [role="main"], .container, .content')
          .first()
          .isVisible()
          .catch(() => false);
    ...
    const firstProjectId = Array.isArray(projectsApi.json)
      ? projectsApi.json[0]?.id
      : null;
    if (firstProjectId) {
      const dynamicRoutes = [
        `/projects/${firstProjectId}`,
        `/projects/${firstProjectId}/overview`,
        `/projects/${firstProjectId}/tasks`,
        `/projects/${firstProjectId}/risks`,
        `/projects/${firstProjectId}/milestones`,
        `/projects/${firstProjectId}/documents`,
        `/projects/${firstProjectId}/budget`,
        `/projects/${firstProjectId}/team`,
        `/projects/${firstProjectId}/edit`,
      ...
      try {
        await page.goto(`${PROD_URL}/projects/${firstProjectId}/overview`, {
          waitUntil: "domcontentloaded",
        });
        await page.waitForLoadState("networkidle");
        ...
        results.components.push({
          route: `/projects/${firstProjectId}/overview`,
          charts: hasChart,
          table: hasTable,
        });
      } catch {}
      ...
      results.tabs.push({
        note: "No projects in production; dynamic routes skipped",
      });
    }
    ...
    try {
      ...
      await page.waitForLoadState("networkidle");
      const textBudget = await page
        .locator("text=งบประมาณรวม")
        .first()
        .locator("xpath=..")
        .locator("text=฿")
        .first()
        ...
        .catch(() => "");
      results.dashboard = {
        budgetText: textBudget?.trim() || null,
        apiRows: Array.isArray(portfolio.json?.rows)
          ? portfolio.json.rows.length
          : null,
      };
    } catch {}
    ...
    for (const t of results.tabs) {
      if (t.route)
        md += `- ${t.route}: ${t.ok ? "OK" : "FAILED"}${t.hasTabs ? " (tabs present)" : ""}\n`;
      else md += `- ${t.note}\n`;
    }
    md += `\n## Components\n`;
