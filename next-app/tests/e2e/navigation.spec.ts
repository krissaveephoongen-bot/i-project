...
test('Navigate main menus load without errors', async ({ page }) => {
  ...
  await page.addInitScript((id) => {
   ...
   const pages = ['/', '/projects', '/tasks', '/timesheet', '/sales', '/reports/projects', '/reports/utilization', '/reports/hours', '/help', '/settings'];
   for (const p of pages) {
     await page.goto(`${base}${p}`);
     await expect(page).toHaveTitle(/I-PROJECT|Projects|Tasks|Timesheet|Sales|Reports|Help|Settings/i);
   }
 });
