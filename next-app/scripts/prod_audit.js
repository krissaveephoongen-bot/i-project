...
function analyzePortfolio(rows = []) {
  ...
  for (const r of rows) {
    ...
    if (budget < 0 || committed < 0 || actual < 0 || remaining < 0) {
      issues.push(
        `Negative values for project ${r.id}: budget=${budget}, committed=${committed}, actual=${actual}, remaining=${remaining}`,
      );
    }
    if (spi < 0 || spi > 5) {
    ...
    if (remaining > budget) {
      issues.push(
        `Remaining exceeds Budget for project ${r.id}: remaining=${remaining}, budget=${budget}`,
      );
    }
  }
  return {
    totals: { budget: b, committed: c, actual: a, remaining: rm },
    issues,
  };
}
...
async function main() {
  ...
  results.push({
    ...
    count: Array.isArray(activities.json) ? activities.json.length : null,
  });

  const exec = await getJson("/api/projects/executive-report");
  results.push({
    name: "projects/executive-report",
    ok: exec.ok,
    status: exec.status,
  });
  ...
  results.push({
    name: "projects/weekly-summary",
    ok: weekly.ok,
    status: weekly.status,
    count: weekly.json?.summary?.length ?? null,
  });
