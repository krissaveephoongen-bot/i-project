...
export default function FilterTestPage() {
  ...
  const filteredData = useMemo(() => {
    ...
    return projectsWithDisplay.filter((item) => {
      ...
      const matchesPriority =
        priorityFilter === "all" || item.displayPriority === priorityFilter;
      const matchesManager =
        managerFilter === "all" || item.displayManager === managerFilter;
  ...
  const FilterComponent = useEnhanced
    ? ProfessionalFilterEnhanced
    : ProfessionalFilter;
    ...
    <div className="container mx-auto py-8 space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">
          Comprehensive testing of the professional filter system with error
        ...
        <CardContent>
          ...
          <div className="mt-4 p-4 bg-muted rounded-lg">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
              ...
              <div>
                Data:{" "}
                {dataLoading ? "Loading..." : dataError ? "Error" : "Loaded"}
              </div>
              <div>Simulated Error: {showError ? "Yes" : "No"}</div>
              <div>
                Results: {filteredData.length}/{projects.length}
              </div>
            </div>
          </div>
        </CardContent>
