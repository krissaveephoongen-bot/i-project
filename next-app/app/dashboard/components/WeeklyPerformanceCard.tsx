        ...
        <table className="w-full text-sm" aria-label="ตารางผลงานรายสัปดาห์">
          <thead>
            <tr className="border-b border-border">
              <th scope="col" className="text-left py-3 px-2 font-medium text-muted-foreground">โครงการ</th>
              <th scope="col" className="text-right py-3 px-2 font-medium text-muted-foreground">Actual</th>
              <th scope="col" className="text-right py-3 px-2 font-medium text-muted-foreground">Plan</th>
              <th scope="col" className="text-right py-3 px-2 font-medium text-muted-foreground">SPI</th>
              <th scope="col" className="text-right py-3 px-2 font-medium text-muted-foreground">เปลี่ยนแปลง</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border/50">
            {data.slice(0, 6).map((w) => (
              <tr key={w.id} className="hover:bg-muted/50 transition-colors">
                ...
                <td className="py-3 px-2 text-right text-muted-foreground">{Number(w.progress || 0).toFixed(1)}%</td>
                <td className="py-3 px-2 text-right text-muted-foreground">{Number((w.progress || 0) / (w.spi || 1)).toFixed(1)}%</td>
            ...
            {data.length === 0 && (
              <tr>
                <td colSpan={5} className="py-8 text-center text-muted-foreground text-sm">ไม่มีข้อมูล</td>
              </tr>
            )}
          </tbody>
        </table>
