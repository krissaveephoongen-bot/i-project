...
function DashboardKPIs({ totals }: DashboardKPIsProps) {
  return (
    <section
      aria-label="ตัวชี้วัดหลัก (KPIs)"
      className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 gap-y-6"
    >
      ...
      <div className="bg-card text-card-foreground rounded-2xl p-5 shadow-sm border border-border relative overflow-hidden group hover:shadow-md transition-all hover:-translate-y-1 duration-300">
        ...
        <div className="flex flex-col relative z-10">
          ...
          <div
            className="w-full bg-muted h-1 mt-3 rounded-full overflow-hidden"
            role="progressbar"
            aria-valuenow={Math.round(
              (totals.committed / (totals.budget || 1)) * 100,
            )}
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label="สัดส่วนผูกพันงบ"
          >
            <div
              className="bg-purple-500 h-full rounded-full transition-all duration-1000 ease-out"
              style={{
                width: `${Math.min((totals.committed / (totals.budget || 1)) * 100, 100)}%`,
              }}
            />
          </div>
        </div>
      ...
      <div className="bg-card text-card-foreground rounded-2xl p-5 shadow-sm border border-border relative overflow-hidden group hover:shadow-md transition-all hover:-translate-y-1 duration-300">
        ...
        <div className="flex flex-col relative z-10">
          ...
          <div
            ...
            role="progressbar"
            aria-valuenow={Math.round(
              (totals.actual / (totals.budget || 1)) * 100,
            )}
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label="สัดส่วนจ่ายจริง"
          >
            <div
              className="bg-blue-500 h-full rounded-full transition-all duration-1000 ease-out"
              style={{
                width: `${Math.min((totals.actual / (totals.budget || 1)) * 100, 100)}%`,
              }}
            />
          </div>
        </div>
      ...
      <div className="bg-card text-card-foreground rounded-2xl p-5 shadow-sm border border-border relative overflow-hidden group hover:shadow-md transition-all hover:-translate-y-1 duration-300">
        ...
        <div className="flex flex-col relative z-10">
          ...
          <div className="flex items-end gap-2">
            <span
              className={`text-2xl font-bold ${totals.avgSpi >= 1 ? "text-emerald-600" : "text-amber-500"}`}
            >
