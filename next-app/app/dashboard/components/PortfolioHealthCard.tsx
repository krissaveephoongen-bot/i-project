...
function PortfolioHealthCard({ data }: PortfolioHealthCardProps) {
  ...
  return (
    <div className="bg-card text-card-foreground rounded-2xl shadow-sm border border-border p-6 hover:shadow-md transition-all">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-bold">
            เมทริกซ์สุขภาพพอร์ตโฟลิโอ (Portfolio Health)
          </h3>
          <p className="text-sm text-muted-foreground">
            วิเคราะห์ประสิทธิภาพ: ต้นทุน (CPI) vs เวลา (SPI)
          </p>
        </div>
      ...
      {data.length === 0 ? (
        <div
          className="flex items-center justify-center h-[400px] text-muted-foreground text-sm"
          role="status"
        >
        ...
        <div
          role="img"
          aria-label={`เมทริกซ์แสดงสุขภาพ ${data.length} โครงการตามค่า CPI และ SPI`}
        >
                    ...
                    return (
                      <div className="bg-card text-card-foreground p-3 border border-border shadow-lg rounded-xl z-50">
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                          ...
                          <span
                            className={`font-medium ${d.spi < 0.9 ? "text-red-600" : ""}`}
                          >
                            {d.spi.toFixed(2)}
                          </span>
                          <span className="text-muted-foreground">CPI:</span>
                          <span
                            className={`font-medium ${d.cpi < 0.9 ? "text-red-600" : ""}`}
                          >
                            {d.cpi.toFixed(2)}
                          </span>
              ...
              >
                {data.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={
                      entry.spi < 0.9 || entry.cpi < 0.9
                        ? "#ef4444"
                        : entry.spi >= 1 && entry.cpi >= 1
                          ? "#10b981"
                          : "#f59e0b"
                    }
