...
function DashboardFilters({
  ...
  const handleExportCSV = () => {
    ...
    const rowsCsv = filteredRows
      .map((r) =>
        cols.map((c) => String(r[c as keyof typeof r] ?? "")).join(","),
      )
      .join("\n");
    const csv = header + "\n" + rowsCsv;
    ...
    <div className="bg-card text-card-foreground rounded-2xl shadow-sm border border-border p-4 flex flex-wrap items-center gap-3 lg:gap-4 transition-all hover:shadow-md">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <Select value={status} onValueChange={setStatus}>
          <SelectTrigger
            className="w-[180px] bg-muted border-border rounded-xl text-sm"
        ...
        <div className="flex items-center gap-2 bg-muted border border-border rounded-xl p-1">
          <label className="sr-only" htmlFor="filter-start-month">
            เดือนเริ่มต้น
          </label>
          <input
          ...
          <label className="sr-only" htmlFor="filter-end-month">
            เดือนสิ้นสุด
          </label>
          <input
            id="filter-end-month"
            ...
            <DialogContent className="max-w-md">
              <div className="space-y-4">
                ...
                <p className="text-sm text-muted-foreground">
                  การดำเนินการนี้จะล้างข้อมูลแคชทั้งหมดและรีเฟรชข้อมูลใหม่
