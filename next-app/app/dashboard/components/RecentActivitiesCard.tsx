...
function RecentActivitiesCard({ activities }: RecentActivitiesCardProps) {
  return (
    <div className="bg-card text-card-foreground rounded-2xl shadow-sm border border-border p-6 flex flex-col hover:shadow-md transition-all">
      ...
      <div className="flex-1 overflow-y-auto space-y-6 pr-2 max-h-[400px]" role="feed" aria-label="กิจกรรมล่าสุด">
        {activities.map((act) => (
          <article key={act.id} className="flex gap-4 group">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
                ...
                <time dateTime={act.date}>{new Date(act.date).toLocaleDateString()}</time>
              </div>
            </div>
          </article>
        ))}
        {activities.length === 0 && (
          <div className="text-center text-muted-foreground py-8 text-sm" role="status">ไม่มีกิจกรรมล่าสุด</div>
        )}
      </div>
    </div>
  );
