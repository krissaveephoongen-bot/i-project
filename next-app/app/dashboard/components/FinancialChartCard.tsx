    ...
    <div className="bg-card text-card-foreground rounded-2xl shadow-sm border border-border p-6 hover:shadow-md transition-all">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">เปรียบเทียบ ยอดผูกพัน (Committed) vs จ่ายจริง (Paid)</p>
        </div>
      ...
      {data.length === 0 ? (
        ...
        <div role="img" aria-label={`กราฟแท่งแสดงข้อมูลการเงิน ${data.length} เดือน`}>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              <YAxis className="text-muted-foreground" fontSize={12} tickFormatter={(v) => `฿${(v / 1000000).toFixed(1)}M`} tickLine={false} axisLine={false} />
              <Tooltip
