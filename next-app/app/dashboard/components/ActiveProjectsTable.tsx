    ...
    <div className="bg-card text-card-foreground rounded-2xl shadow-sm border border-border overflow-hidden hover:shadow-md transition-all">
      <div className="p-6 border-b border-border flex items-center justify-between">
        <h3 className="text-lg font-bold">
          โครงการที่ดำเนินการอยู่ (Active Projects)
        </h3>
          ...
          <div key={r.id} className="p-4 space-y-3">
            <div className="flex items-start justify-between">
              <div>
                <Link
                  ...
                  className="font-semibold hover:text-blue-600 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring rounded"
                >
                  {r.name}
                </Link>
                <span
                  className={`text-xs mt-1 block w-fit px-2 py-0.5 rounded-full ${STATUS_COLORS[r.status?.toLowerCase()] || "bg-muted text-muted-foreground"}`}
                >
                  {r.status || "Active"}
                </span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div>
                ...
                <span className="font-medium">{r.progress?.toFixed(0)}%</span>
              </div>
              <div>
                <span className="text-muted-foreground">SPI:</span>{" "}
                <span className="font-medium">{(r.spi || 1).toFixed(2)}</span>
              </div>
              <div>
              ...
              <div>
                <span className="text-muted-foreground">จ่ายจริง:</span>{" "}
        ...
        <table className="w-full" aria-label="ตารางโครงการที่ดำเนินการอยู่">
          <thead className="bg-muted/50">
            <tr>
              <th
                scope="col"
                className="text-left py-4 px-6 text-xs font-semibold text-muted-foreground uppercase tracking-wider"
              >
                ชื่อโครงการ
              </th>
              <th
                scope="col"
                className="text-center py-4 px-6 text-xs font-semibold text-muted-foreground uppercase tracking-wider"
              >
                ความคืบหน้า
              </th>
              <th
              ...
              <th
                scope="col"
                className="text-right py-4 px-6 text-xs font-semibold text-muted-foreground uppercase tracking-wider"
              >
                งบประมาณ
              </th>
              <th
              ...
              <tr key={r.id} className="hover:bg-muted/50 transition-all group">
                <td className="py-4 px-6">
                  <div className="flex flex-col">
                    ...
                    <span
                      className={`text-xs mt-1 w-fit px-2 py-0.5 rounded-full ${STATUS_COLORS[r.status?.toLowerCase()] || "bg-muted text-muted-foreground"}`}
                    >
                      {r.status || "Active"}
                    </span>
                  </div>
                ...
                <td className="py-4 px-6">
                  <div className="flex flex-col items-center gap-1">
                    ...
                    <div
                      ...
                      role="progressbar"
                      aria-valuenow={Math.round(r.progress || 0)}
                      aria-valuemin={0}
                      aria-valuemax={100}
                      aria-label={`ความคืบหน้า ${r.name}`}
                    >
                      <div
                ...
                <td className="py-4 px-6">
                  <div className="flex justify-center gap-2">
                    {r.risks?.high > 0 && (
                      <span
                        className="px-2 py-1 bg-red-100 text-red-700 dark:bg-red-950/30 dark:text-red-400 text-xs font-bold rounded flex items-center gap-1"
                        aria-label={`ความเสี่ยงสูง ${r.risks.high} รายการ`}
                      >
                        H:{r.risks.high}
                      </span>
                    )}
                    {r.risks?.medium > 0 && (
                      <span
                        className="px-2 py-1 bg-orange-100 text-orange-700 dark:bg-orange-950/30 dark:text-orange-400 text-xs font-bold rounded flex items-center gap-1"
                        aria-label={`ความเสี่ยงปานกลาง ${r.risks.medium} รายการ`}
                      >
                        M:{r.risks.medium}
                      </span>
                    )}
