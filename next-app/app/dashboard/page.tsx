...
const FinancialChartCard = createLazyComponent(
  () => import("./components/FinancialChartCard"),
  {
...
const TrendChartCard = createLazyComponent(
  () => import("./components/TrendChartCard"),
  {
    loadingVariant: "chart",
    loadingClassName: "h-80",
  },
...
const PortfolioHealthCard = createLazyComponent(
  () => import("./components/PortfolioHealthCard"),
  {
    loadingVariant: "card",
  },
);

const WeeklyPerformanceCard = createLazyComponent(
  () => import("./components/WeeklyPerformanceCard"),
  {
    loadingVariant: "card",
  },
);

const ExecutiveSummaryCard = createLazyComponent(
  () => import("./components/ExecutiveSummaryCard"),
  {
    loadingVariant: "card",
  },
);

const RecentActivitiesCard = createLazyComponent(
  () => import("./components/RecentActivitiesCard"),
  {
    loadingVariant: "list",
    loadingCount: 5,
  },
...
const ActiveProjectsTable = createLazyComponent(
  () => import("./components/ActiveProjectsTable"),
  {
...
export default function UnifiedDashboard() {
  ...
  const fetchDashboardData = async (forceRefresh = false) => {
    ...
    try {
      ...
      const [pf, act, er, ws] = await Promise.all([
        fetch(`/api/dashboard/portfolio?${timestamp}`, {
          cache: "no-store",
          signal,
        }),
        fetch(`/api/dashboard/activities?${timestamp}`, {
          cache: "no-store",
          signal,
        }),
        fetch(`/api/projects/executive-report?${timestamp}`, {
          cache: "no-store",
          signal,
        }).catch(() => ({ ok: false })),
        fetch(`/api/projects/weekly-summary?${timestamp}`, {
          cache: "no-store",
          signal,
        }).catch(() => ({ ok: false })),
      ]);
      ...
      const pfJson = pf.ok
        ? await (pf as Response).json()
        : { rows: [], cashflow: [], spiTrend: [], spiSnaps: [] };
      console.log(
        "Dashboard: API Response - rows count:",
        (pfJson.rows || []).length,
      );
      console.log(
        "Dashboard: API Response - sample row:",
        (pfJson.rows || [])[0],
      );
      setRows(pfJson.rows || []);
      ...
      const wsJson = (ws as Response).ok
        ? await (ws as Response).json()
        : { summary: [] };
  ...
  const totals = useMemo<DashboardTotals>(() => {
    ...
    const avgSpi = rows.length
      ? rows.reduce((s, r) => s + Number(r.spi || 1), 0) / rows.length
      : 1;
  ...
  const filteredTotals = useMemo<DashboardTotals>(() => {
    ...
    const committed = filteredRows.reduce(
      (s, r) => s + Number(r.committed || 0),
      0,
    );
    ...
    const avgSpi = filteredRows.length
      ? filteredRows.reduce((s, r) => s + Number(r.spi || 1), 0) /
        filteredRows.length
      : 1;
    return { budget, committed, actual, remaining, completedCount, avgSpi };
  }, [filteredRows]);
  ...
  const filteredSpiTrend = useMemo(() => {
    if (selectedSpiProject !== "all") {
      const filteredSnaps = spiSnaps.filter(
        (s) => s.projectId === selectedSpiProject,
      );
      filteredSnaps.sort((a, b) => a.date.localeCompare(b.date));

      return filteredSnaps
        .map((s) => ({ date: s.date, spi: s.spi }))
        .filter((s) => {
  ...
  const summaryCards = useMemo<DashboardSummary>(() => {
    const completedProjects = filteredRows.filter(
      (r) => Number(r.progress || 0) >= 100,
    ).length;
    const inProgressProjects = filteredRows.filter((r) => {
      const p = Number(r.progress || 0);
      return p > 0 && p < 100;
    }).length;
    const highRisks = filteredRows.reduce(
      (s, r) => s + Number(r.risks?.high || 0),
      0,
    );
    const overdueMilestones = filteredRows.reduce(
      (s, r) => s + Number(r.overdueMilestones || 0),
      0,
    );
    return {
      completedProjects,
  ...
  if (loading) {
    return (
      <div
        className="min-h-screen bg-background p-4 lg:p-8 pt-20 lg:pt-24 space-y-8"
        role="status"
  ...
  if (error) {
    return (
      <div
        className="min-h-screen flex flex-col items-center justify-center bg-background gap-6 p-4"
        role="alert"
      >
  ...
  return (
    <PageTransition className="min-h-screen bg-background">
      ...
      <div className="pt-20 lg:pt-24 px-4 lg:px-8 pb-12 max-w-[1600px] mx-auto space-y-8">
        {rows.length === 0 && (
          <div
            className="bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/50 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4"
            role="status"
          >
            <div className="flex items-center gap-4">
              <div
                className="w-12 h-12 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center shrink-0"
                aria-hidden="true"
              >
              ...
              <div className="text-center sm:text-left">
                ...
                <p className="text-blue-700 dark:text-blue-400">
                  สร้างโครงการแรกเพื่อเริ่มติดตามข้อมูลเชิงลึกที่นี่
                </p>
              </div>
            </div>
            <Link
