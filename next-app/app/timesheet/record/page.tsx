...
export default function TimesheetRecordPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Load Projects
      try {
        const projs = await getTimesheetProjectsAction(user.id);
        setProjects(projs || []);
      ...
      try {
        ...
        setTeamLoad({
          weeklyCapacity: 40,
          currentLoad: loadedEntries.reduce(
  ...
  const createEntry = async () => {
    ...
    try {
      ...
      const entryData: Partial<TimeEntry> = {
        ...
        taskId:
          form.workType === WorkType.PROJECT && form.taskMode === "planned"
            ? form.taskId || null
            : null,
        billableHours:
          form.workType === WorkType.PROJECT && form.billable ? form.hours : 0,
      };

      const newEntry = await createTimesheetEntryAction(entryData);

      if (newEntry) {
        setEntries((prev) => [newEntry as unknown as TimeEntry, ...prev]);
  ...
  const deleteEntry = async (id: string) => {
    ...
    const success = await deleteTimesheetEntryAction(id);
    if (success) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <Header
        title="บันทึกข้อมูลการทำงาน (Timesheet)"
        breadcrumbs={[
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  <div className="flex justify-between text-sm mb-1">
                    <span>
                      Weekly Hours: {teamLoad.currentLoad} /{" "}
                      {teamLoad.weeklyCapacity} hrs
                    </span>
                    <span className="font-medium">
                      {Math.round(
                        (teamLoad.currentLoad / teamLoad.weeklyCapacity) * 100,
                      )}
                      % Utilization
                    </span>
                  </div>
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value={WorkType.OFFICE}>
                    Non-Project (Admin, Meeting, etc.)
                  </SelectItem>
                  <SelectItem value={WorkType.LEAVE}>
                    Leave (ลาป่วย/ลากิจ)
                  </SelectItem>
                </SelectContent>
              </Select>
                  ...
                  {form.taskMode === "planned" && (
                    <div className="md:col-span-2">
                      <Label className="mb-1.5 block">
                        เลือกงาน (Task) <span className="text-red-500">*</span>
                      </Label>
                      <Select
                        value={form.taskId}
                ...
                <div className="md:col-span-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      {activities
                        .filter(
                          (a) => a.workType === form.workType && a.isActive,
                        )
                        .map((a) => (
              ...
              {form.workType === WorkType.PROJECT && (
                <div className="md:col-span-2 flex items-center gap-2">
                  ...
                  <Label htmlFor="billable" className="cursor-pointer">
                    Billable (คิดเป็นค่าใช้จ่ายโครงการ)
                  </Label>
                </div>
              )}
            </div>
                      ...
                      <td className="py-3 px-4">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${
                            e.workType === WorkType.PROJECT
                              ? "bg-blue-100 text-blue-700"
                              : e.workType === WorkType.LEAVE
