...
interface MonthlyViewProps {
  ...
  isEditing: boolean;
  canEdit: boolean;
  onOpenDayEditor: (projectId: string, day: number, taskId?: string | null) => void;
}
...
export default function MonthlyView({ 
  ...
  const hoursMap = useMemo(() => {
    ...
    entries.forEach(e => {
      ...
      projectTotalMap.set(projKey, (projectTotalMap.get(projKey) || 0) + e.hours);
  ...
  const getCellHours = (projectId: string, day: number, taskId?: string | null) => {
  ...
  const grandTotal = useMemo(() => {
    return Array.from(hoursMap.dailyTotalMap.values()).reduce((a, b) => a + b, 0);
  }, [hoursMap]);
        ...
        <Table>
          ...
          <TableBody>
            {projects.map(project => {
               const projectTotal = daysInMonth.reduce((sum, day) => sum + getProjectDayTotal(project.id, day), 0);
                   ...
                   {isExpanded && (
                       <>
                           {projectTasks.map(task => {
                               const taskTotal = daysInMonth.reduce((sum, day) => sum + getCellHours(project.id, day, task.id), 0);
                               ...
                               return (
                                   <TableRow key={`${project.id}-${task.id}`} className="hover:bg-slate-50 transition-colors">
                                       ...
                                       {daysInMonth.map(day => {
                                           const daySum = getCellHours(project.id, day, task.id);

                                           return (
                                               <TableCell key={day} className="p-1 text-center border-r border-slate-50">
                                                   {isEditing && canEdit ? (
                                                       <div 
                                                           onClick={() => onOpenDayEditor(project.id, day, task.id)}
                                                           className={`
                               ...
                               {daysInMonth.map(day => {
                                   ...
                                   return (
                                       <TableCell key={day} className="p-1 text-center border-r border-slate-50">
                                           {isEditing && canEdit ? (
                                               <div 
                                                   onClick={() => onOpenDayEditor(project.id, day, null)}
                                                   className={`
                                                       h-7 w-full min-w-[2rem] rounded-md flex items-center justify-center cursor-pointer text-xs transition-colors
