...
export default function TimesheetModal({
    ...
    const [concurrentWarnings, setConcurrentWarnings] = useState<Record<number, ConcurrentWarning>>({});
    const [concurrentReasons, setConcurrentReasons] = useState<Record<number, string>>({});
    const [confirmedConcurrent, setConfirmedConcurrent] = useState<Set<number>>(new Set());
    ...
    const handleSave = () => {
        const invalidRows = rows.filter(r => !r.project || !r.startTime || !r.endTime);
                    ...
                    {rows.map((row, idx) => (
                        <div key={row.id} className="space-y-2">
                            ...
                            {concurrentWarnings[idx] && (
                                <div className={`p-3 rounded-lg border-l-4 ${concurrentWarnings[idx].requiresComment
                                        ...
                                        <div className="text-sm">
                                            ...
                                            {concurrentWarnings[idx].overlappingEntries && (
                                                <div className="mt-2 pl-2 border-l-2 border-current opacity-75">
                                                    <p className="text-xs font-medium mb-1">{t('timesheet.parallelWork')}:</p>
                                                    {concurrentWarnings[idx].overlappingEntries!.map((e) => (
                                                        <p key={e.id} className="text-xs">
                                                            • {e.projectName} {e.startTime}-{e.endTime} ({formatDuration(e.overlapMinutes / 60).split(' ')[0]}h)
                                                        </p>
                                                    ))}
                                                </div>
                                            )}
                                                ...
                                                <div className="mt-3">
                                                    ...
                                                    <label className="flex items-center gap-2 mt-2 cursor-pointer">
                                                        ...
                                                        <span className="text-xs">
                                                            {t('timesheet.concurrentProjects')}: {concurrentWarnings[idx].overlappingEntries?.length || 1}
                                                        </span>
                                                    </label>
                                                </div>
                                            )}
                            ...
                            <div className="grid grid-cols-12 gap-3 items-start p-4 bg-slate-50 rounded-xl border border-slate-200">
                                ...
                                <div className="col-span-3 space-y-1">
                                    <label className="text-xs font-medium text-slate-700">{t('timesheet.project')} *</label>
                                    <Select
                                        value={row.project || ''}
                                        onValueChange={(val) => updateRow(idx, { project: val, task: '' })}
                                    >
                                ...
                                <div className="col-span-3 space-y-1">
                                    <label className="text-xs font-medium text-slate-700">{t('common.type')}</label>
                                    <Select
                                        value={row.task || 'none'}
                                        onValueChange={(val) => updateRow(idx, { task: val === 'none' ? undefined : val })}
                                    >
                                        <SelectTrigger className="h-10 text-sm bg-white">
                                            <SelectValue placeholder={t('common.select')} />
                                        </SelectTrigger>
                                        <SelectContent>
                                            <SelectItem value="none">-- {t('workTypes.general')} --</SelectItem>
                                            {(projects.find(p => p.id === row.project)?.tasks || []).map(t => (
                                ...
                                <div className="col-span-2 grid grid-cols-2 gap-2">
                                    <div className="space-y-1">
                                        <label className="text-xs font-medium text-slate-700">{t('timesheet.startTime')} *</label>
                                        <Input
                                            type="time"
                                            value={row.startTime}
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-xs font-medium text-slate-700">{t('timesheet.endTime')} *</label>
                                        <Input
                                            type="time"
                                            value={row.endTime}
                                ...
                                <div className="col-span-1 space-y-1">
                                    <label className="text-xs font-medium text-slate-700">{t('timesheet.hours')}</label>
                                ...
                                <div className="col-span-2 space-y-1">
                                    <label className="text-xs font-medium text-slate-700">{t('common.description')}</label>
                                    <Input
