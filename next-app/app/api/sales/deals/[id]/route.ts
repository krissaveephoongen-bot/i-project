...
import { supabase } from '@/app/lib/supabaseClient';
 ...
 export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
   try {
     const id = params.id;
    const { data: d } = await supabase.from('sales_deals').select('*').eq('id', id).limit(1);
    const deal = (d || [])[0];
    if (!deal) return err('Not found', 404);
    let stageName = null;
    if (deal.stage_id) {
      const { data: stage } = await supabase.from('sales_stages').select('name').eq('id', deal.stage_id).limit(1);
 ...
 export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
   try {
    ...
    const { data, error } = await supabase.from('sales_deals').update(payload).eq('id', id).select('*').limit(1);
