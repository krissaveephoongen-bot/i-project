...
export async function GET(_req: NextRequest) {
  try {
    ...
    for (const p of projects) {
      try {
        const res = await fetch(`${base}/api/projects/overview/${p.id}`, {
          cache: "no-store",
        });
        const ov = res.ok ? await res.json() : null;
        if (ov)
          list.push({ id: p.id, name: p.name, status: p.status, overview: ov });
      } catch {}
    }
    ...
    const statusCounts = {
      ...
      planning: list.filter(
        (x) => String(x.status || "").toLowerCase() === "planning",
      ).length,
      completed: list.filter(
        (x) => String(x.status || "").toLowerCase() === "completed",
      ).length,
    };
    const avgSpi = list.length
      ? list.reduce((s, x) => s + Number(x.overview?.project?.spi || 1), 0) /
        list.length
      : 1;
    ...
    const overdueMilestones = list.reduce((s, x) => {
      ...
      const count = (x.overview?.milestones || []).filter((m: any) => {
        ...
        const dt = new Date(d);
        return (
          dt < today && !["paid", "approved", "completed"].includes(status)
        );
      }).length;
      return s + count;
    }, 0);
    const budgetTotals = list.reduce(
      (acc, x) => {
