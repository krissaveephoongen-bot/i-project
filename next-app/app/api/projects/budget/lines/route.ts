  ...
  try {
    ...
    try {
      ...
      const r = await pool.query(
        `SELECT id,name,percentage,status,due_date FROM milestones WHERE project_id=$1`,
        [projectId],
      );
      milestones = r.rows || [];
    }
    ...
    const paid = milestones
      ...
      .map((m) => ({
        id: m.id,
        amount: (Number(m.percentage || 0) / 100) * budget,
    ...
    return NextResponse.json(
      { error: error?.message || "budget lines error" },
      { status: 500 },
    );
  }
}
