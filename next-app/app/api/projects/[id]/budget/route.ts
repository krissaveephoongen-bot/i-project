...
import { firstOk, PROJECT_ID_COLUMNS, USER_ID_COLUMNS, isSchemaColumnError } from '../../../_lib/supabaseCompat';
 ...
 export async function GET(_: Request, { params }: { params: { id: string } }) {
   try {
    ...
    const pRes = await supabase.from('projects').select('id,name,budget,spent').eq('id', projectId).limit(1).single();
    ...
    const msRes = await firstOk(PROJECT_ID_COLUMNS, (col) =>
      supabase.from('milestones').select('id,name,title,amount,percentage,due_date,dueDate,status').eq(col, projectId)
    );
    const milestones = ((msRes as any)?.data || []).map((m: any) => ({
      id: m.id,
      name: m.name ?? m.title ?? '',
    ...
    const teRes = await firstOk(PROJECT_ID_COLUMNS, (col) =>
      supabase.from('time_entries').select('date,hours,user_id,userId,userid').eq(col, projectId)
    );
    ...
    const userIds = Array.from(new Set(entries.map((e: any) => e.userId).filter(Boolean)));
    ...
    if (userIds.length) {
      ...
      for (const sel of candidates) {
        const r = await supabase.from('users').select(sel).in('id', userIds as any);
        if (!r.error) {
          uRows = r.data || [];
          lastErr = null;
          break;
      ...
      if (lastErr && !isSchemaColumnError(lastErr)) return err(lastErr.message, 500);
