...
export async function GET(req: NextRequest) {
  try {
    ...
    const pRes = await pool.query(
      `SELECT progress, "progressPlan", progress_plan, created_at, "created_at" FROM projects WHERE id = $1`,
      [projectId],
    );
    ...
    const created_at = new Date(
      proj.created_at || proj["created_at"] || new Date(),
    );
    ...
    const spi = data.length
      ? data[data.length - 1].actual / Math.max(1, data[data.length - 1].plan)
      : 1;
    ...
    const weeks = 12;
    const planSeries = Array.from({ length: weeks }, (_, i) =>
      Math.min(100, Math.round((100 / weeks) * (i + 1))),
    );
    const actualSeries = Array.from({ length: weeks }, (_, i) =>
      Math.min(100, Math.round((50 / weeks) * (i + 1))),
    );
    const data = planSeries.map((p, i) => ({
      week: weekLabel(startDate, i),
      plan: p,
