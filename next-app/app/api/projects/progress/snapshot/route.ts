...
export async function GET(req: NextRequest) {
  try {
    ...
    const msRes = await firstOk(PROJECT_ID_COLUMNS, (col) =>
      supabase.from("milestones").select("*").eq(col, projectId),
    );
    ...
    const ms = (milestones || []).map((m: any) => {
      const pct =
        m.percentage != null
          ...
          : budget > 0
            ? Math.min(100, (Number(m.amount || 0) / budget) * 100)
            : 0;
      return {
    ...
    const tRes = await firstOk(PROJECT_ID_COLUMNS, (col) =>
      supabase.from("tasks").select("*").eq(col, projectId),
    );
    ...
    const weightOf = (t: any) =>
      Number(t.weight ?? t.estimatedHours ?? t.estimated_hours ?? 1) || 0;
    const totalWeight =
      (tasks || []).reduce((s: number, t: any) => s + weightOf(t), 0) || 1;
    for (const mid of Object.keys(tasksByMilestone)) {
      ...
      const msWeight =
        list.reduce((s: number, t: any) => s + weightOf(t), 0) || 1;
      ...
      if (!list || list.length === 0) {
        ...
        const status = String(
          (milestones || []).find((m: any) => String(m.id) === String(mid))
            ?.status || "",
        ).toLowerCase();
        if (actualDate || status === "paid") {
          const k = startOfWeek(new Date(actualDate || msObj!.due))
            .toISOString()
            .slice(0, 10);
      ...
      for (const t of list) {
        ...
        // Actual allocation
        const progressActual = Number(
          t.progressActual ?? t.progress_actual ?? 0,
        );
    ...
    for (const k of weekKeys) {
      ...
      points.push({
        date: k,
        plan: Number(planCum.toFixed(2)),
        actual: Number(actualCum.toFixed(2)),
        spi,
        milestone: Number(milestoneInc[k] || 0),
      });
    }
    ...
    return NextResponse.json(
      { points: [], error: e?.message || "snapshot error" },
      { status: 200 },
    );
  }
}
