  ...
  try {
    ...
    const performanceMetrics = {
      database: {
        ...
        total_size_mb: tableStats?.reduce((sum: number, table: any) => sum + (table.size_mb || 0), 0) || 0,
        total_rows: tableStats?.reduce((sum: number, table: any) => sum + (table.row_count || 0), 0) || 0,
        total_indexes: tableStats?.reduce((sum: number, table: any) => sum + (table.index_count || 0), 0) || 0,
        largest_table: tableStats?.[0]?.table_name || 'N/A',
        slow_queries: slowQueries.length,
        avg_query_time: slowQueries.reduce((sum: number, q: any) => sum + parseFloat(q.avg_time), 0) / slowQueries.length || 0
      },
      cache: {
    ...
    if (parseFloat(cacheData?.hit_rate || '0%') < 80) {
      recommendations.push({
        type: 'cache',
        priority: 'medium',
        message: 'Cache hit rate is below 80%, consider adjusting TTL strategies',
