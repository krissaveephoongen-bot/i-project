...
export async function POST() {
  try {
    ...
    const { data: timeResult, error: timeError } = await supabase
      .from('time_entries')
      .delete()
      .lt('date', new Date(Date.now() - 730 * 24 * 60 * 60 * 1000).toISOString());
    ...
    const { data: progressResult, error: progressError } = await supabase
      ...
      .delete()
      .lt('date', new Date(Date.now() - 365 * 24 * 60 * 60 * 1000).toISOString());
    ...
    const { data: spiResult, error: spiError } = await supabase
      ...
      .delete()
      .lt('date', new Date(Date.now() - 180 * 24 * 60 * 60 * 1000).toISOString());
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      stats: cleanupStats,
      recommendations: [
        'Run cleanup to free up ' + cleanupStats.total_deletable + ' old records',
