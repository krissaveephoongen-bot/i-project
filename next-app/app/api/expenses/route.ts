...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await client
      .from("expenses")
      .select(
        "id, user_id, user_id, project_id, project_id, task_id, task_id, date, amount, category, description, status, rejected_reason, rejectedReason, approved_by, approvedBy, approved_at, approvedAt, approver_id, receiptUrl, details, project:projects(id, name), task:tasks(id, title), approver:users!approver_id(id, name)",
      )
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Cannot edit processed expense" },
        { status: 403 },
      );
    }
