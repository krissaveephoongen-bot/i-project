...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!t1.error) {
      ...
      console.warn(
        "Timesheet fetch with date failed, trying work_date...",
        t1.error,
      );
      const t2 = await supabaseAdmin
        .from("time_entries")
        .select("*")
        .gte("work_date" as any, date30DaysAgoStr);
      if (!t2.error) {
    ...
    const totalHoursLogged = userStats.reduce(
      (sum: number, u: any) => sum + u.total_hours,
      0,
    );
    const avgUtilization =
      totalTeamMembers > 0
        ? userStats.reduce((sum: number, u: any) => sum + u.utilization, 0) /
          totalTeamMembers
        : 0;
