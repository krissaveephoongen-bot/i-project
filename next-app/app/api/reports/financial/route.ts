...
export async function GET(req: NextRequest) {
  try {
    ...
    const totalRevenue = (visibleProjects || []).reduce(
      (sum: number, p: any) => sum + Number(p.budget || 0),
      0,
    );
    const totalCost = (visibleProjects || []).reduce(
      (sum: number, p: any) => sum + Number(p.spent || 0),
      0,
    );
    ...
    return NextResponse.json(
      { projects: visibleProjects, kpis, chartData },
      { status: 200 },
    );
  } catch (e: any) {
