...
export async function GET(req: NextRequest) {
  try {
    ...
    const overBudget = (visibleProjects || []).filter((p: any) => Number(p.spent || 0) > Number(p.budget || 0)).length;
    
    const avgProgress = totalProjects > 0 ? (visibleProjects || []).reduce((sum: number, p: any) => sum + Number(p.progress || 0), 0) / totalProjects : 0;
    ...
    return NextResponse.json({ projects: visibleProjects, kpis }, { status: 200 });
  } catch (e: any) {
