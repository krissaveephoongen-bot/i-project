...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      if (id) {
        const res = await supabase.from('contacts').select('*').eq('id', id).limit(1).single();
        if (res.error) {
      ...
      const rows = (res.data || []).filter((c: any) => ['stakeholder', 'client', 'Stakeholder', 'Client'].includes(String(c.type || '')));
      return ok(rows, 200);
    } catch (e: any) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, position, email, phone, project_id, projectId, type, organization, role } = body;
...
export async function PUT(req: NextRequest) {
  try {
    const body = await req.json();
    const { id, name, position, email, phone, type, project_id, projectId, is_key_person, organization, role } = body;
    ...
    const r2 = await supabase.from('stakeholders').update(fallback).eq('id', id);
    if (r2.error) return err(r2.error.message, 500);
