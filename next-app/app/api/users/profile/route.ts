...
export async function GET(request: NextRequest) {
  try {
    ...
    const myProjects = (allProjects || []).filter(
      (p: any) => String(p.managerId || "") === String(userId || ""),
    );
    const myTasks = (allTasks || []).filter(
      (t: any) => String(t.assignedTo || "") === String(userId || ""),
    );
    const stats = {
      totalProjects: myProjects.length,
      activeProjects: myProjects.filter((p: any) =>
        ["in_progress", "active", "ongoing"].includes(
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq("id", userId)
      .select(
        "id,name,email,role,status,department,position,avatar,phone,timezone,is_active,is_deleted,hourly_rate,employee_code,name_th,created_at,updated_at",
      )
      .limit(1);
    if (error) return err(error.message || "update failed", 500);
    return ok(
      { user: (data || [])[0] || {}, message: "Profile updated successfully" },
      200,
    );
  } catch (error: any) {
    return err(error?.message || "Internal server error", 500);
