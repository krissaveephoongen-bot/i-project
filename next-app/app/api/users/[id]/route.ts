...
export async function PATCH(
  ...
  try {
    ...
    if (typeof parsed.employeeCode !== 'undefined') payload.employeeCode = String(parsed.employeeCode);
    if (typeof parsed.phone !== 'undefined') payload.phone = parsed.phone;
    if (typeof parsed.timezone !== 'undefined') payload.timezone = parsed.timezone;
    if (typeof parsed.hourlyRate !== 'undefined') payload.hourlyRate = parsed.hourlyRate;
    if (typeof parsed.isActive !== 'undefined') payload.isActive = parsed.isActive;
    if (typeof parsed.isDeleted !== 'undefined') payload.isDeleted = parsed.isDeleted;
    ...
    const { data, error } = await supabase
      ...
      .update(payload)
      .eq('id', userId)
      .select('id,name,email,role,status,employeeCode,isActive,isDeleted,timezone,hourlyRate,updatedAt')
      .limit(1);
    if (error) return err(error.message || 'update failed', 500);
    return ok({ user: (data || [])[0] || {}, message: 'User updated successfully' }, 200);
