  ...
  try {
    if (!supabaseAdmin)
      return NextResponse.json(
        { ok: false, error: "admin client missing" },
        { status: 200 },
      );
    ...
    const { data: users } = await supabaseAdmin
      ...
      .eq("email", "jakgrits.ph@appworks.co.th")
      .limit(1);
    const admin = (users || [])[0];
    if (!admin)
      return NextResponse.json(
        { ok: false, error: "seed user not found" },
        { status: 200 },
      );
    ...
    await supabaseAdmin
      ...
      .upsert({
        id: clientId,
        name: "AppWorks Test Client",
        email: "client@appworks.co.th",
      });
    ...
    await supabaseAdmin.from("milestones").upsert([
      {
        ...
        percentage: 25,
        status: "approved",
        due_date: addMonths(today, -2).toISOString(),
      },
      {
        id: m2,
        project_id: projectId,
        ...
        percentage: 25,
        status: "paid",
        due_date: addMonths(today, -1).toISOString(),
      },
      {
        id: m3,
        project_id: projectId,
        ...
        percentage: 20,
        status: "approved",
        due_date: addMonths(today, 0).toISOString(),
      },
      {
        id: m4,
        project_id: projectId,
    ...
    await supabaseAdmin
      ...
      .upsert(snaps, { onConflict: "id" });

    return NextResponse.json(
      { ok: true, projectId, taskIds: [task1, task2] },
      { status: 200 },
    );
  } catch (e: any) {
    return NextResponse.json(
