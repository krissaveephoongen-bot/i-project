...
import { Plus, Edit2, Trash2, DollarSign, AlertCircle, RefreshCw, Eye } from 'lucide-react';
...
export default function ExpensesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [expensesRes, projectsRes] = await Promise.all([
        ...
        fetch('/api/projects?userId=' + user.id, { cache: 'no-store' }).catch(() => ({ ok: false }))
      ]);
  ...
  const categoryBreakdown = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([cat, amount]) => ({ category: cat, amount }));
  }, [expenses]);
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 gap-6 p-4">
        ...
        <div className="text-center max-w-md space-y-2">
          <h3 className="text-xl font-bold text-slate-900">Error Loading Expenses</h3>
      ...
      <div className="pt-24 px-8 pb-12 max-w-[1400px] mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-slate-500 mt-1">{summary.count} expenses</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-600">฿{summary.approved.toLocaleString()}</div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-600">฿{summary.rejected.toLocaleString()}</div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1">Track and manage expense claims</p>
          </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-lg font-semibold">{detailExpense.date}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-slate-500">Amount</label>
                    <p className="text-lg font-semibold">฿{Number(detailExpense.amount).toLocaleString()}</p>
                  </div>
                </div>

                <div>
                  <label className="text-sm font-medium text-slate-500">Project</label>
                  <p className="text-lg font-semibold">{detailExpense.project?.name}</p>
                </div>

                <div>
                  <label className="text-sm font-medium text-slate-500">Category</label>
                  <p className="text-lg font-semibold">{detailExpense.category}</p>
                </div>
                ...
                <div>
                  ...
                  <Badge 
                    className="mt-2"
                    variant={
                      detailExpense.status === 'approved' ? 'default' :
                      detailExpense.status === 'rejected' ? 'destructive' :
                      'secondary'
                    }
                  >
                ...
                {detailExpense.description && (
                  <div>
                    ...
                    <p className="text-sm text-slate-600 mt-1">{detailExpense.description}</p>
                  </div>
                )}
                ...
                {detailExpense.rejectedReason && (
                  <div className="bg-red-50 border border-red-200 rounded p-3">
                    ...
                    <p className="text-sm text-red-700 mt-1">{detailExpense.rejectedReason}</p>
                  </div>
                )}
