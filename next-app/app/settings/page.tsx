...
export default function SettingsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const res = await fetch(`/api/users/profile?${param}`, {
        cache: "no-store",
      });
    ...
    <PageTransition>
      <div className="min-h-screen bg-slate-50/50">
        <Header
          ...
          breadcrumbs={[{ label: "แดชบอร์ด", href: "/" }, { label: "โปรไฟล์" }]}
        />
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-6">
                    <div className="relative">
                      <img
                        src={
                          (user as any)?.avatar ||
                          `https://ui-avatars.com/api/?name=${encodeURIComponent(form.name || "User")}&background=2563EB&color=fff&size=128`
                        }
                        className="w-24 h-24 rounded-full shadow-md border-4 border-white ring-1 ring-slate-100 object-cover"
                        alt="Avatar"
                      />
                      <div
                        className={`absolute bottom-1 right-1 w-5 h-5 border-2 border-white rounded-full ${(user as any)?.is_active !== false ? "bg-green-500" : "bg-slate-400"}`}
                      ></div>
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <Card>
                  ...
                  <CardContent>
                    ...
                    <p className="text-xs text-muted-foreground">
                      hours logged{" "}
                      {filter !== "all" ? "in this period" : "all time"}
                    </p>
                  </CardContent>
                </Card>
                <Card>
            ...
            <TabsContent value="edit">
              <Card className="shadow-sm border-slate-200">
                <CardHeader>
                  ...
                  <CardDescription>
                    จัดการข้อมูลส่วนตัวและรายละเอียดการติดต่อของคุณ
                  </CardDescription>
                </CardHeader>
                <CardContent>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-sm font-medium flex items-center gap-2 text-slate-700">
                          <User className="w-4 h-4 text-slate-400" /> ชื่อ
                          (ภาษาอังกฤษ)
                        </label>
                        <Input
                          value={form.name}
