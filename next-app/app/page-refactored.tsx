...
async function getKpiData() {
  const { data: projects, error: projectsError } = await supabase.from('projects').select('budget, status');
  if (projectsError) throw projectsError;

  const totalValue = projects.reduce((sum: number, p: any) => sum + (p.budget || 0), 0);

  const { count: activeIssues } = await supabase.from('risks').select('*', { count: 'exact', head: true }).in('status', ['open', 'in_progress']);

  const firstDayOfMonth = new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString();
  const lastDayOfMonth = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).toISOString();
  const { data: milestones, error: milestonesError } = await supabase.from('milestones').select('amount').gte('due_date', firstDayOfMonth).lte('due_date', lastDayOfMonth);
  if (milestonesError) throw milestonesError;

  const billingForecast = milestones.reduce((sum: number, m: any) => sum + (m.amount || 0), 0);
...
async function getProjectsHealth() {
  ...
  const { data: risks, error: risksError } = await supabase.from('risks').select('project_id, status').in('status', ['open', 'in_progress']);
  if(risksError) throw risksError;

  const risksPerProject = risks.reduce((acc: {[key: string]: number}, risk: any) => {
...
async function getFinancialData() {
  const { data, error } = await supabase.from('financial_data').select('*').order('month', { ascending: true });
  if (error) {
    console.error("Error fetching financial data:", error);
    return [];
  }
  return data.map((d: any) => ({...d, month: new Date(d.month).toLocaleDateString('en-US', { month: 'short' })}));
}
...
async function getTeamLoad() {
    ...
    const userHours: { [key: string]: { name: string; mon: number; tue: number; wed: number; thu: number; fri: number; } } = {};
    data.forEach((entry: any) => {
        const user = entry.users as any;
        if (!user) return;

        if (!userHours[user.id]) {
            userHours[user.id] = { name: user.name, mon: 0, tue: 0, wed: 0, thu: 0, fri: 0 };
        }
        const day = new Date(entry.date).getUTCDay();
        const dayMap: Array<'sun' | 'mon' | 'tue' | 'wed' | 'thu' | 'fri' | 'sat'> = ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'];
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-6 py-8 pt-24">
        ...
        <div className="flex flex-col space-y-4 sm:flex-row sm:justify-between sm:items-start sm:space-y-0 mb-6 sm:mb-8">
          ...
          <Button variant="outline" className="w-full sm:w-auto gap-2 self-start">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="text-center">{t('projects.columns.risk')}</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                        ...
                        <TableRow key={project.id} className="hover:bg-muted/50">
                          <TableCell>
                            <div className="flex items-center space-x-3">
                              <Avatar className="h-8 w-8">
                                <AvatarImage src={`https://api.dicebear.com/7.x/initials/svg?seed=${project.name}`} />
                          ...
                          <TableCell className="text-center">
                            <div className="flex items-center space-x-2">
                              <Progress value={project.progress_actual} className="w-16" />
                              <span className="text-sm font-medium">{project.progress_actual}%</span>
                            </div>
                          </TableCell>
                          <TableCell className="text-center">
                            <Badge variant={project.spi >= 1 ? 'default' : 'destructive'}>
                              {project.spi >= 1 ? (
