...
import { useTasks, useProjects, useTimesheets } from '../../hooks/useSupabaseData';
...
export default function StaffPortalPage() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'tasks' | 'timesheet' | 'projects'>('dashboard');
  const router = useRouter();

  // Use real database hooks
  const { data: tasks, loading: tasksLoading, error: tasksError } = useTasks(user?.id);
  const { data: projects, loading: projectsLoading } = useProjects();
  const { data: timeEntries, loading: timesheetsLoading } = useTimesheets(user?.id);
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <aside className="hidden md:fixed md:left-0 md:top-0 md:h-screen md:w-[260px] md:bg-[#0F172A] md:text-white md:flex md:flex-col md:z-50">
        ...
        <div className="p-4 border-b border-slate-700">
          <p className="text-xs text-slate-400 uppercase tracking-wider mb-2">Staff Portal</p>
        </div>
        <nav className="flex-1 py-4 px-3 space-y-1">
          <button
            ...
            className={clsx(
              ...
              activeTab === 'dashboard' ? 'bg-[#2563EB]' : 'text-slate-300 hover:bg-slate-700'
            )}
          >
            Dashboard
          </button>
          <button
            ...
            className={clsx(
              'w-full px-4 py-2 rounded-lg text-sm font-medium transition-colors text-left',
              activeTab === 'tasks' ? 'bg-[#2563EB]' : 'text-slate-300 hover:bg-slate-700'
            )}
          >
            My Tasks
          </button>
          <button
            ...
            className={clsx(
              'w-full px-4 py-2 rounded-lg text-sm font-medium transition-colors text-left',
              activeTab === 'timesheet' ? 'bg-[#2563EB]' : 'text-slate-300 hover:bg-slate-700'
            )}
          >
            Timesheet
          </button>
          <button
            ...
            className={clsx(
              'w-full px-4 py-2 rounded-lg text-sm font-medium transition-colors text-left',
              activeTab === 'projects' ? 'bg-[#2563EB]' : 'text-slate-300 hover:bg-slate-700'
            )}
          >
            Projects
          </button>
        ...
        <div className="p-4 border-t border-slate-700">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-400">{user?.email || 'staff@company.com'}</p>
            </div>
          </div>
          <button
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 bg-black/50 z-40" onClick={() => setMobileMenuOpen(false)}>
          <aside className="absolute right-0 top-0 h-screen w-[260px] bg-[#0F172A] text-white p-4" onClick={(e) => e.stopPropagation()}>
          ...
          {activeTab === 'dashboard' && (
            <div>
              ...
              <div className="bg-gradient-to-r from-[#2563EB] to-[#7C3AED] rounded-xl p-6 mb-6 text-white">
                <h1 className="text-xl md:text-2xl font-bold mb-2">สวัสดี, {user?.name || 'Staff'}!</h1>
                <p className="text-blue-100 text-sm">นี่คือภาพรวมของงานและโปรเจคของคุณ</p>
              </div>
                      ...
                      <div key={task.id} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                        ...
                        <span className={clsx(
                          ...
                          {task.status === 'completed' ? 'เสร็จ' :
                           task.status === 'in_progress' ? 'กำลังทำ' : 'รอดำเนินการ'}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200">
                  ...
                  <div className="p-4">
                    <div className="text-center mb-4">
                      ...
                      <p className="text-sm text-slate-600">ชั่วโมงที่ทำสัปดาห์นี้</p>
                    </div>
                    <div className="space-y-2">
                      {timeEntries?.slice(0, 3).map((entry) => (
                        <div key={entry.id} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                          <div className="flex-1">
                            <p className="text-sm font-medium text-slate-900">{entry.task}</p>
                            <p className="text-xs text-slate-500">{entry.project}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-sm font-medium text-slate-900">{entry.hours}h</p>
                            <span className={clsx(
                              'text-xs',
                              ...
                              {entry.status === 'approved' ? 'อนุมัติ' :
                               entry.status === 'pending' ? 'รออนุมัติ' : 'ปฏิเสธ'}
                            </span>
                          </div>
                        </div>
                      ))}
                          ...
                          <div className="flex-1 h-2 bg-slate-200 rounded-full overflow-hidden">
                            <div 
                              className={clsx(
                                ...
                                task.progress === 100 ? 'bg-green-500' : 'bg-[#2563EB]'
                              )}
                              style={{ width: `${task.progress}%` }}
                            />
                          </div>
                          <span className="text-xs font-medium text-slate-600 w-10">{task.progress}%</span>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {activeTab === 'timesheet' && (
            <div className="bg-white rounded-xl shadow-sm border border-slate-200">
              <div className="px-4 py-4 border-b border-slate-200 flex items-center justify-between">
                <h3 className="text-lg font-semibold text-slate-900">Timesheet</h3>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <h4 className="text-sm font-medium text-slate-900">{entry.tasks?.title || 'No Task'}</h4>
                          <span className={clsx(
                            'px-2 py-0.5 rounded text-xs',
                ...
                <div key={project.id} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between text-xs text-slate-500">
                      <span>Start: {project.start_date ? new Date(project.start_date).toLocaleDateString() : 'No start date'}</span>
