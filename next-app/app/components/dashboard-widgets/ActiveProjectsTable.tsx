            ...
            {projects.length === 0 ? (
              ...
              projects.map((p) => (
                <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={clsx("font-medium", p.spi < 0.9 ? "text-red-600" : "text-green-600")}>
                      {p.spi.toFixed(2)}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex gap-1">
                      {p.risks.high > 0 && <span className="w-2 h-2 rounded-full bg-red-500" title="High Risk" />}
                      {p.risks.medium > 0 && <span className="w-2 h-2 rounded-full bg-orange-500" title="Medium Risk" />}
                      {p.risks.low > 0 && <span className="w-2 h-2 rounded-full bg-blue-500" title="Low Risk" />}
                      {p.risks.high === 0 && p.risks.medium === 0 && p.risks.low === 0 && (
