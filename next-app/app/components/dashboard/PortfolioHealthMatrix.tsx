      ...
      return (
        <div className="bg-white p-3 border border-slate-200 rounded-lg shadow-lg text-xs">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1">
            ...
            <span className={clsx("font-mono font-medium", data.spi >= 1 ? "text-green-600" : "text-red-600")}>{data.spi}</span>
            <span className="text-slate-500">CPI:</span>
            <span className={clsx("font-mono font-medium", data.cpi >= 1 ? "text-green-600" : "text-red-600")}>{data.cpi}</span>
            <span className="text-slate-500">Progress:</span>
            <span className="font-mono">{data.progress_actual}%</span>
            <span className="text-slate-500">Budget:</span>
            <span className="font-mono">฿{(data.budget / 1000000).toFixed(1)}M</span>
          </div>
        </div>
      );
    }
        ...
        <div className="flex bg-white rounded-lg border border-slate-200 p-0.5">
          <button
            ...
            className={clsx(
              ...
              viewMode === 'matrix' ? "bg-slate-100 text-blue-600 shadow-sm" : "text-slate-400 hover:text-slate-600"
            )}
            title="Matrix View"
          >
            ...
            <ResponsiveContainer width="100%" height="100%" className="col-start-1 col-span-3 row-start-1 row-span-3">
              ...
              >
                ...
                <XAxis 
                  ...
                  dataKey="x" 
                  name="SPI" 
                  label={{ value: 'Schedule Performance Index (SPI)', position: 'bottom', offset: 0, fontSize: 12 }} 
                  domain={[0, 2]} 
                  ticks={[0, 0.5, 1, 1.5, 2]}
                />
                <YAxis 
                  ...
                  dataKey="y" 
                  name="CPI" 
                  label={{ value: 'Cost Performance Index (CPI)', angle: -90, position: 'left', offset: 0, fontSize: 12 }} 
                  domain={[0, 2]}
                  ticks={[0, 0.5, 1, 1.5, 2]}
                />
                ...
                <ReferenceLine x={1} stroke="#94a3b8" strokeDasharray="3 3" strokeWidth={2} />
                <ReferenceLine y={1} stroke="#94a3b8" strokeDasharray="3 3" strokeWidth={2} />
                
                <Scatter name="Projects" data={data} fill="#8884d8">
                  {data.map((entry, index) => {
                    // Determine color based on Quadrant
                    let color = '#94a3b8'; // default
                    if (entry.spi >= 1 && entry.cpi >= 1) color = '#10b981'; // Green (Star)
                    else if (entry.spi < 1 && entry.cpi >= 1) color = '#f59e0b'; // Amber (Potential)
                    else if (entry.spi >= 1 && entry.cpi < 1) color = '#f97316'; // Orange (Overrun)
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  <th className="text-left py-3 px-4 text-sm font-semibold text-slate-600 rounded-tl-lg">Project</th>
                  <th className="text-center py-3 px-4 text-sm font-semibold text-slate-600">SPI</th>
                  <th className="text-center py-3 px-4 text-sm font-semibold text-slate-600">CPI</th>
                  <th className="text-center py-3 px-4 text-sm font-semibold text-slate-600">Progress</th>
                  <th className="text-center py-3 px-4 text-sm font-semibold text-slate-600 rounded-tr-lg">Risk</th>
                ...
                {filtered.map((project, idx) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-center">
                      <Badge variant="outline" className={clsx("font-mono", project.spi >= 1 ? "bg-green-50 text-green-700 border-green-200" : "bg-red-50 text-red-700 border-red-200")}>
                        {project.spi}
                      </Badge>
                    </td>
                    <td className="py-3 px-4 text-center">
                      <Badge variant="outline" className={clsx("font-mono", project.cpi >= 1 ? "bg-green-50 text-green-700 border-green-200" : "bg-red-50 text-red-700 border-red-200")}>
                        {project.cpi}
                      </Badge>
                    </td>
                    ...
                    <td className="py-3 px-4 text-center">
                      <Badge variant="outline" className={getRiskColor(project.risk_level)}>
                        {project.risk_level}
                      </Badge>
                    </td>
                    <td className="py-3 px-4 text-right">
                      <Link href={`/projects/${project.id}`} className="text-blue-600 hover:text-blue-800 text-sm font-medium inline-flex items-center gap-1">
