...
export default function ReportsSunburst({ data }: { data: Node }) {
  ...
  const levels = useMemo(() => {
    const l0 = (current.children || []).map((c) => ({
      name: c.name,
      value: c.value,
      children: c.children,
    }));
    const l1 = (l0[0]?.children || []).map((c) => ({
      name: c.name,
      value: c.value,
      children: c.children,
    }));
    const l2 = (l1[0]?.children || []).map((c) => ({
      name: c.name,
      value: c.value,
      meta: (c as any).meta,
    }));
  ...
  const onSliceClick = (levelIndex: number, entry: any) => {
    if (levelIndex === 0) setPath([entry.name]);
      ...
      setPath([current.children?.[0]?.name || "", entry.name]);
    else setPath([...path]);
  };
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-lg font-semibold text-slate-900">
          Project Insight Analytics (Sunburst)
        </h3>
        <button
          onClick={onZoomOut}
      ...
      <div className="w-full flex justify-center">
        <PieChart width={800} height={480}>
          <Pie
            ...
            innerRadius={100}
            outerRadius={160}
            onClick={(_, i) => onSliceClick(0, levels[0][i])}
          >
            {(levels[0] || []).map((_, i) => (
          ...
          <Pie
            ...
            innerRadius={170}
            outerRadius={230}
            onClick={(_, i) => onSliceClick(1, levels[1][i])}
          >
            {(levels[1] || []).map((_, i) => (
          ...
          <Tooltip
            formatter={(value: any, name: any, p: any) => {
              ...
              const start = n?.meta?.start
                ? new Date(n.meta.start).toLocaleString()
                : "-";
              const end = n?.meta?.end
                ? new Date(n.meta.end).toLocaleString()
                : "-";
      ...
      <div className="mt-3 text-xs text-slate-600">
        เลื่อนหรือใช้เมาส์คลิกที่ส่วนเพื่อ Zoom In / Zoom Out
        และเอาเมาส์ไปชี้เพื่อดูเวลาเริ่ม-สิ้นสุด
      </div>
    </div>
  );
}
