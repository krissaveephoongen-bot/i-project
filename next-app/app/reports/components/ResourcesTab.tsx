...
import { getResourcesReportAction } from "../actions";
...
export default function ResourcesTab() {
  ...
  useEffect(() => {
    ...
    getResourcesReportAction()
      .then((data) => {
        // Transform action result to match component state structure if needed
        // Action returns { resources, utilization }
        // Component expects { userStats, kpis }
        setData({
          userStats: data.resources.map((r) => ({
            id: r.id,
            name: r.name,
            position: r.role || r.department || "Staff",
            total_hours: r.activeTasks * 10, // Mock hours or real data if available
            utilization: r.allocation,
            avatar: null,
          })),
          kpis: {
            totalTeamMembers: data.resources.length,
            totalHoursLogged: data.resources.length * 160 * (data.utilization.avg / 100), // Estimate
            averageUtilization: data.utilization.avg,
          },
        });
      })
      .catch((error) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">
              Total Hours Logged (30d)
            </CardTitle>
            <Clock className="h-4 w-4 text-green-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {kpis.totalHoursLogged.toLocaleString()} hrs
            </div>
          </CardContent>
        </Card>
        <Card>
