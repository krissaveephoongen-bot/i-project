  ...
  useEffect(() => {
    ...
    getFinancialReportAction()
      .then((res) => {
        ...
        // Action returns { summary, projects, trends }
        // Component expects { projects, kpis, chartData }
        setData({
          projects: res.projects,
          kpis: {
            totalRevenue: res.summary.totalBudget,
            totalCost: res.summary.totalSpent,
            netProfit: res.summary.remaining,
            margin: res.summary.profitMargin.toFixed(1),
          },
          chartData: res.trends.map((t) => ({
            name: t.month,
            revenue: t.revenue,
            cost: t.cost,
          })),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-end gap-2">
        <Label htmlFor="include-internal-fin" className="text-slate-600">
          รวม Internal
        </Label>
        <Switch
          id="include-internal-fin"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div
              className={`text-2xl font-bold ${kpis.netProfit >= 0 ? "text-blue-600" : "text-red-600"}`}
            >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {projects.map((p: any) => {
                  const profit =
                    (Number(p.budget) || 0) - (Number(p.spent) || 0);
                  const margin =
                    Number(p.budget) > 0
                      ? (profit / Number(p.budget)) * 100
                      : 0;
                  return (
                    <tr key={p.id} className="border-b hover:bg-slate-50">
                      ...
                      <td
                        className={`py-3 px-4 text-right font-medium ${profit >= 0 ? "text-blue-600" : "text-red-600"}`}
                      >
