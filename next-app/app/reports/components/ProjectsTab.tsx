...
export default function ProjectsTab() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/reports/projects${q}`, { cache: 'no-store' })
      .then(res => res.json())
      .then(setData)
      .catch(console.error)
  ...
  return (
    <div className="space-y-6">
       <div className="flex items-center justify-end gap-2">
         <Label htmlFor="include-internal" className="text-slate-600">รวม Internal</Label>
         <Switch id="include-internal" checked={includeInternal} onCheckedChange={setIncludeInternal} />
       </div>
       {/* KPIs */}
       <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card>
             ...
             <CardContent>
                <div className="text-2xl font-bold text-blue-600">{kpis.avgProgress}%</div>
             </CardContent>
          </Card>
          <Card>
             <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium text-slate-600">Completed On Time</CardTitle>
                <CheckCircle2 className="h-4 w-4 text-green-500" />
             </CardHeader>
             <CardContent>
                <div className="text-2xl font-bold text-green-600">{kpis.onTime}</div>
             </CardContent>
          </Card>
          <Card>
             <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium text-slate-600">Over Budget</CardTitle>
                <AlertTriangle className="h-4 w-4 text-red-500" />
             </CardHeader>
             <CardContent>
                <div className="text-2xl font-bold text-red-600">{kpis.overBudget}</div>
             </CardContent>
          </Card>
       </div>
          ...
          <CardContent className="h-[400px]">
             <ResponsiveContainer width="100%" height="100%">
                <BarChart data={projects} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                   ...
                   <Tooltip />
                   <Legend />
                   <Bar dataKey="progress" name="Actual Progress" fill="#2563eb" radius={[4, 4, 0, 0]} />
                   <Bar dataKey="progressPlan" name="Planned Progress" fill="#94a3b8" radius={[4, 4, 0, 0]} />
                </BarChart>
             </ResponsiveContainer>
          </CardContent>
       </Card>
                   ...
                   <tbody>
                      {projects.map((p: any) => (
                         <tr key={p.id} className="border-b hover:bg-slate-50">
                            ...
                            <td className="py-3 px-4"><Badge variant="outline">{p.status}</Badge></td>
                            <td className="py-3 px-4 text-right">{p.progress}%</td>
                            <td className="py-3 px-4 text-right">฿{Number(p.budget).toLocaleString()}</td>
                            <td className="py-3 px-4 text-right">฿{Number(p.spent).toLocaleString()}</td>
                         </tr>
                      ))}
                   </tbody>
                </table>
