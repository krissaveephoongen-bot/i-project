...
export default function EditMilestonePage() {
  ...
  const projectId =
    typeof params?.id === "string"
      ...
      : Array.isArray(params?.id)
        ...
        : "";
  const mid =
    typeof params?.mid === "string"
      ? (params!.mid as string)
      : Array.isArray(params?.mid)
        ? (params!.mid as string[])[0]
        : "";
  ...
  const [planReceived, setPlanReceived] = useState<string>(
    milestone?.plan_received_date || "",
  );
  ...
  const [newTask, setNewTask] = useState<{
    ...
    startDate: string;
    endDate: string;
  }>({ name: "", weight: 0, startDate: "", endDate: "" });

  useEffect(() => {
    const loadTasks = async () => {
      const res = await fetch(
        `${API_BASE}/api/projects/tasks?milestoneId=${mid}&projectId=${projectId}`,
      );
      ...
      <div className="container mx-auto px-6 py-8 pt-24 max-w-2xl">
        <div className="space-y-4 bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="block">
              <span className="text-sm text-slate-600">
                วันคาดว่าจะรับเงิน (Plan Received)
              </span>
              <input
                type="date"
                value={planReceived}
                onChange={(e) => setPlanReceived(e.target.value)}
                className="mt-1 w-full border rounded px-3 py-2"
              />
            </label>
            <label className="block">
              <span className="text-sm text-slate-600">
                วันรับชำระจริง (Receipt Date)
              </span>
              <input
                type="date"
                value={receipt}
            ...
            <Button
              onClick={() =>
                save({
                  ...
                  plan_received_date: planReceived,
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200">
                ...
                <th className="text-center py-2 px-3 text-xs font-semibold text-slate-600">
                  น้ำหนัก (%)
                </th>
                <th className="text-center py-2 px-3 text-xs font-semibold text-slate-600">
                  เริ่ม
            ...
            <tbody>
                  ...
                  <td className="py-2 px-3 text-center">
                    <input
                      ...
                      onBlur={async (e) => {
                        ...
                        await fetch(`${API_BASE}/api/projects/tasks/update`, {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({
                            id: t.id,
                    ...
                    <input
                      ...
                      onBlur={async (e) => {
                        await fetch(`${API_BASE}/api/projects/tasks/update`, {
                          ...
                          body: JSON.stringify({
                            id: t.id,
                            updatedFields: { start_date: e.target.value },
                          }),
                        });
                      }}
                    />
                    ...
                    <input
                      ...
                      onBlur={async (e) => {
                        await fetch(`${API_BASE}/api/projects/tasks/update`, {
                          ...
                          body: JSON.stringify({
                            id: t.id,
                            updatedFields: { end_date: e.target.value },
                          }),
                        });
                      }}
                    />
                    ...
                    <input
                      ...
                      onBlur={async (e) => {
                        ...
                        await fetch(`${API_BASE}/api/projects/tasks/update`, {
                          ...
                          body: JSON.stringify({
                            id: t.id,
                            updatedFields: { progress_actual: v },
                          }),
                        });
                      }}
                    />
              ...
              {tasks.length === 0 && (
                <tr>
                  <td className="py-3 px-3 text-sm text-slate-500" colSpan={6}>
                    ยังไม่มีงานในงวดนี้
                  </td>
                </tr>
              )}
                ...
                <td className="py-2 px-3 text-center" colSpan={2}>
                  <Button
                    onClick={async () => {
                      ...
                      const res = await fetch(
                        `${API_BASE}/api/projects/tasks/create`,
                        {
                          method: "POST",
                          headers: { "Content-Type": "application/json" },
                          body: JSON.stringify({
