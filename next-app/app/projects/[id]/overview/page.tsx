                ...
                <div className="pt-24 px-6 pb-6 container mx-auto space-y-6 w-full max-w-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <Card className="border-0 shadow-md hover:shadow-lg transition-shadow duration-300 bg-gradient-to-br from-blue-50/80 to-white">
                            ...
                            <CardContent>
                                <div className="text-3xl font-bold text-blue-600">{Number(p?.progress || 0).toFixed(1)}%</div>
                                <Progress value={Number(p?.progress || 0)} className="h-2 mt-3" />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <Card className="border-0 shadow-md hover:shadow-lg transition-shadow duration-300 bg-gradient-to-br from-slate-50/80 to-white">
                            ...
                            <CardContent>
                                <div className="text-3xl font-bold text-slate-900">฿{Number(summary?.totalBudget || 0).toLocaleString()}</div>
                        ...
                        <Card className="border-0 shadow-md hover:shadow-lg transition-shadow duration-300 bg-gradient-to-br from-purple-50/80 to-white">
                            ...
                            <CardContent>
                                <div className="text-3xl font-bold text-purple-600">฿{Number(summary?.committedCost || 0).toLocaleString()}</div>
                        ...
                        <Card className="border-0 shadow-md hover:shadow-lg transition-shadow duration-300 bg-gradient-to-br from-red-50/80 to-white">
                            ...
                            <CardContent>
                                <div className="text-3xl font-bold text-red-600">฿{Number(summary?.actualCost || 0).toLocaleString()}</div>
                        ...
                        <Card className="border-0 shadow-md hover:shadow-lg transition-shadow duration-300 bg-gradient-to-br from-green-50/80 to-white">
                            ...
                            <CardContent>
                                <div className="text-3xl font-bold text-green-600">฿{Number(summary?.remainingBudget || 0).toLocaleString()}</div>
                                <p className="text-xs text-slate-500 mt-2">คงเหลือจากงบประมาณ</p>
                            </CardContent>
                        </Card>
                    </div>
                            ...
                            <CardContent className="min-h-[200px] pt-4">
                                <div className="space-y-3">
                                    {(risks || []).slice(0, 5).map((r: any) => (
                                        <div key={r.id} className="flex items-center justify-between p-3 bg-gradient-to-r from-red-50/50 to-transparent rounded-lg border border-red-100/50 hover:border-red-200 transition-colors">
                                    ...
                                    {(risks || []).length === 0 && <p className="text-sm text-slate-500 text-center py-8">ไม่มีความเสี่ยงที่บันทึกไว้</p>}
                                </div>
                            </CardContent>
                        </Card>
                                    ...
                                    {(milestones || []).slice(0, 5).map((m: any) => (
                                        <div key={m.id} className="space-y-1.5">
                                            <div className="flex justify-between text-sm">
                                                ...
                                                <span className="text-sm font-semibold text-blue-600">{Number(m.percentage || 0)}%</span>
                                            </div>
                                            <Progress value={Number(m.percentage || 0)} className="h-2.5 bg-slate-100" />
                                        </div>
                                    ))}
                                    {(milestones || []).length === 0 && <p className="text-sm text-slate-500 text-center py-8">ไม่มีไมล์สโตนที่กำหนดไว้</p>}
                                </div>
                            </CardContent>
                        </Card>
                    </div>
