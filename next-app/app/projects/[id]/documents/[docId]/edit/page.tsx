...
export default function EditDocumentPage() {
  ...
  const docId = typeof params?.docId === 'string' ? (params!.docId as string) : Array.isArray(params?.docId) ? (params!.docId as string[])[0] : '';
  ...
  const doc = useMemo(() => (data || []).find((d: any) => d.id === docId) || null, [data, docId]);
        ...
        <Card className="w-full max-w-md">
          <CardContent className="pt-6">
            <div className="text-center space-y-4">
              ...
              <Button onClick={() => router.push(`/projects/${projectId}/documents`)}>Back</Button>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="container mx-auto px-6 py-8 pt-24 max-w-2xl">
        <div className="space-y-4 bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          <label className="block">
            ...
            <input value={url} onChange={(e)=>setUrl(e.target.value)} className="mt-1 w-full border rounded px-3 py-2" />
          </label>
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={()=>router.push(`/projects/${projectId}/documents`)}>กลับ</Button>
