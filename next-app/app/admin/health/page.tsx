...
export default function AdminHealthPage() {
  ...
  const labels = {
    ...
    subtitle: language === 'th' ? 'จุดเดียวสำหรับตรวจสอบสถานะระบบทั้งหมด' : 'Single point monitoring for all system components',
      ...
      if (redisHealth.status === 'fulfilled') {
        ...
        components.push({
          ...
          metrics: {
            ...
            opsPerSec: data?.redis?.operations?.instantaneous_ops_per_sec || '0'
          },
          lastChecked: new Date().toISOString(),
          ...
          {error && (
            <div className="bg-red-50 border border-red-200 rounded p-4">
              <div className="flex items-center gap-2">
                ...
                <div className="text-red-700 text-sm">{labels.error}: {error}</div>
              </div>
            </div>
          )}
              ...
              <CardContent>
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right text-sm text-slate-500">
                    <div>{labels.lastChecked}: {new Date(healthData.timestamp).toLocaleTimeString()}</div>
                    <div>{labels.uptime}: {healthData.uptime}</div>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          {healthData && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {healthData.components.map((component) => (
                <Card key={component.name} className="shadow-sm border-slate-200">
                  <CardHeader>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        <div className="text-sm font-medium text-slate-700">{labels.metrics}:</div>
          ...
          <Card className="shadow-sm border-slate-200">
            ...
            <CardContent>
              <div className="flex flex-wrap gap-3">
                <Button
                  onClick={() => window.open('/admin/cache-dashboard', '_blank')}
                  className="gap-2"
                >
