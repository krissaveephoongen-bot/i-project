...
async function fetchAllApprovals(): Promise<ApprovalRequest[]> {
    try {
        ...
        // Fetch pending leave
        const leaveRes = await fetch(`${API_BASE}/api/leave/requests/for-approval`, {
            headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
        });
...
export default function ApprovalStatusPage() {
    ...
    return (
        <div className="min-h-screen bg-slate-50/50">
            <Header
                title={t('timesheet.approvalPending') || 'สรุปสถานะคำร้องการอนุมัติทั้งหมด'}
                breadcrumbs={[
            ...
            <div className="container mx-auto px-6 py-8 pt-24 max-w-7xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    <Card>
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-medium text-slate-600">{t('common.total')}</CardTitle>
                        </CardHeader>
                        <CardContent>
                    ...
                    <Card className="border-yellow-200 bg-yellow-50/50">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-medium text-yellow-700">{t('common.pending')}</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="text-3xl font-bold text-yellow-700">{stats.pending}</div>
                            <p className="text-xs text-yellow-600 mt-1">{t('timesheet.approvalPending')}</p>
                        </CardContent>
                    </Card>

                    <Card className="border-green-200 bg-green-50/50">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-medium text-green-700">{t('common.approved')}</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="text-3xl font-bold text-green-700">{stats.approved}</div>
                            <p className="text-xs text-green-600 mt-1">{t('timesheet.approvalApproved')}</p>
                        </CardContent>
                    </Card>

                    <Card className="border-red-200 bg-red-50/50">
                        <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-medium text-red-700">{t('common.rejected')}</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="text-3xl font-bold text-red-700">{stats.rejected}</div>
                            <p className="text-xs text-red-600 mt-1">{t('timesheet.approvalRejected')}</p>
                        </CardContent>
                    </Card>
                </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-4">
                            ...
                            <Select value={typeFilter} onValueChange={(v: any) => setTypeFilter(v)}>
                                ...
                                <SelectContent>
                                    ...
                                    <SelectItem value="timesheet">{t('timesheet.title')}</SelectItem>
                                    <SelectItem value="expense">{t('navigation.expenses')}</SelectItem>
                                    <SelectItem value="leave">{t('navigation.approvals')}</SelectItem>
                                </SelectContent>
                            </Select>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                <thead className="bg-slate-50">
                                    <tr>
                                        <th className="text-left py-3 px-6 text-sm font-medium text-slate-600">{t('common.type')}</th>
                                        <th className="text-left py-3 px-6 text-sm font-medium text-slate-600">{t('common.name')}</th>
                                        <th className="text-left py-3 px-6 text-sm font-medium text-slate-600">{t('common.date')}</th>
                                        <th className="text-left py-3 px-6 text-sm font-medium text-slate-600">{t('common.total')}</th>
                                        <th className="text-left py-3 px-6 text-sm font-medium text-slate-600">{t('common.status')}</th>
                                        <th className="text-left py-3 px-6 text-sm font-medium text-slate-600">{t('common.date')}</th>
                                        <th className="text-right py-3 px-6 text-sm font-medium text-slate-600">{t('common.actions')}</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100">
                                    {isLoading ? (
                                        ...
                                        filtered.map((item) => (
                                            <tr key={`${item.type}-${item.id}`} className={`hover:bg-slate-50 transition-colors ${item.isConcurrent ? 'bg-yellow-50/30' : ''}`}>
                                                ...
                                                <td className="py-3 px-6 text-sm text-slate-600">
                                                    {item.amount ? `฿${item.amount.toLocaleString()}` : item.hours ? `${item.hours} ${t('timesheet.hours')}` : '-'}
                                                </td>
                                                <td className="py-3 px-6">
                ...
                <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <p className="text-sm text-blue-900">
                        <strong>{t('common.total')}:</strong> {t('timesheet.filterByStatus')} {filtered.length} {t('common.of')} {stats.total}
                        {stats.pending > 0 && (
                            <span className="block mt-1">
                                ⚠️ {t('timesheet.maxConcurrentProjects')} <strong>{stats.pending}</strong>
                            </span>
                        )}
                    </p>
                </div>
