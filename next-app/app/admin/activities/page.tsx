...
export default function ActivityAdminPage() {
  ...
  const openModal = (activity?: ActivityType) => {
    setCurrentActivity(activity || { isActive: true, workType: WorkType.PROJECT });
    setModalOpen(true);
  };

  return (
    <div className="min-h-screen bg-slate-50">
      <Header title="จัดการประเภทกิจกรรม (Activity Types)" breadcrumbs={[{ label: 'Admin', href: '/admin' }, { label: 'Activities' }]} />
      
      <div className="pt-24 px-6 pb-12 max-w-7xl mx-auto space-y-6">
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-slate-800">รายการประเภทกิจกรรม</h2>
                  ...
                  <TableRow key={activity.id}>
                    ...
                    <TableCell>
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        activity.workType === WorkType.PROJECT ? 'bg-blue-100 text-blue-700' :
                        activity.workType === WorkType.LEAVE ? 'bg-red-100 text-red-700' :
                    ...
                    <TableCell>
                      <span className={`px-2 py-1 rounded-full text-xs ${activity.isActive ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
                        {activity.isActive ? 'Active' : 'Inactive'}
                      </span>
                    </TableCell>
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="icon" onClick={() => openModal(activity)}>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{currentActivity.id ? 'แก้ไขประเภทกิจกรรม' : 'เพิ่มประเภทกิจกรรม'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Select 
                ...
                onValueChange={(v: WorkType) => setCurrentActivity({...currentActivity, workType: v})}
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  ...
                  <SelectItem value={WorkType.OFFICE}>Office / Non-Project</SelectItem>
            ...
            <div className="flex items-center gap-2">
              <input 
                ...
                onChange={e => setCurrentActivity({...currentActivity, isActive: e.target.checked})}
