...
export default function AdminCostCodesPage() {
  ...
  const update = async () => {
    try {
      const res = await fetch(
        `/api/admin/cost-codes/${encodeURIComponent(editing.code)}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const remove = async (code: string) => {
    ...
    try {
      const res = await fetch(
        `/api/admin/cost-codes/${encodeURIComponent(code)}`,
        { method: "DELETE" },
      );
      const json = await res.json();
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="pt-24 px-6 pb-12 max-w-5xl mx-auto space-y-6">
        <div className="flex justify-between items-center">
          ...
          <Button onClick={openCreate} className="gap-2">
            <Plus className="w-4 h-4" /> เพิ่ม Cost Code
          </Button>
        </div>
              ...
              <TableBody>
                {codes.map((c: any) => (
                  <TableRow key={c.code}>
                    ...
                    <TableCell>
                      <span
                        className={`px-2 py-1 rounded-full text-xs ${c.is_active ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-600"}`}
                      >
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editing ? "แก้ไข Cost Code" : "เพิ่ม Cost Code"}
            </DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={editing ? update : create}>
