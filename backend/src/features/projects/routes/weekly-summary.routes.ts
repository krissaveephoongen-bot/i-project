...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const weeklySummary =
        await weeklySummaryService.getWeeklySummary(filters);
...
router.get("/compare/:weekStart", async (req, res) => {
  try {
    ...
    const comparison = await weeklySummaryService.compareWeeks(
      new Date(weekStart),
    );
    res.json(comparison);
  } catch (error) {
...
router.get("/trends/:year", async (req, res) => {
  try {
    ...
    const trends = await weeklySummaryService.getWeeklyTrends(
      parseInt(year),
      weeks,
    );
    res.json(trends);
...
router.get("/available-weeks/:year", async (req, res) => {
  try {
    ...
    const availableWeeks = await weeklySummaryService.getAvailableWeeks(
      parseInt(year),
    );
