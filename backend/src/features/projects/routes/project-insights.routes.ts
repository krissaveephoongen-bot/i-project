  ...
  async (req, res, next) => {
    try {
      ...
      const analysis: ProjectStructureAnalysis =
        await insightsService.getProjectStructureAnalysis(year, month, filters);
      res.json(analysis);
    } catch (error) {
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const analysis: TimesheetAnalysis =
        await insightsService.getTimesheetAnalysis(filters);
      res.json(analysis);
    } catch (error) {
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const comparison: InsightComparison =
        await insightsService.compareInsights(period1, period2, filters || {});
