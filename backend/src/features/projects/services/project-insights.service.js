...
export class ProjectInsightsService {
  ...
  async getProjectStructureAnalysis(year, month, filters = {}) {
    ...
    if (month) {
      whereConditions.push(sql`EXTRACT(MONTH FROM ${timeEntries.date}) = ${parseInt(month)}`);
    }
    ...
    const projectCount = new Set(data.filter(entry => entry.projectId).map(entry => entry.projectId)).size;
  ...
  async getTimesheetAnalysis(filters = {}) {
    ...
    const workTypeBreakdown = {
      project: data.filter(entry => entry.workType === 'project').reduce((sum, entry) => sum + Number(entry.hours), 0),
      office: data.filter(entry => entry.workType === 'office').reduce((sum, entry) => sum + Number(entry.hours), 0),
      other: data.filter(entry => entry.workType === 'other').reduce((sum, entry) => sum + Number(entry.hours), 0),
    };
    ...
    data.forEach(entry => {
      if (entry.projectId && entry.workType === 'project') {
        const current = projectMap.get(entry.projectId) || { name: entry.projectName || 'Unknown', hours: 0 };
    ...
    const projectBreakdown = Array.from(projectMap.entries()).map(([id, data]) => ({
      ...
      hours: data.hours,
      percentage: totalProjectHours > 0 ? (data.hours / totalProjectHours) * 100 : 0,
    }));
    ...
    const staffBreakdown = Array.from(staffMap.entries()).map(([userId, data]) => ({
      userId,
      userName: data.name,
      hours: data.total,
    ...
    const timeDistribution = new Array(24).fill(0).map((_, hour) => ({ hour, count: 0 }));
  ...
  async getProjectInsightSummary(filters = {}) {
    ...
    data.forEach(entry => {
      ...
      const userCurrent = userMap.get(entry.userId) || { name: entry.userName || 'Unknown', hours: 0 };
    ...
    const mostActiveProject = projectEntries.reduce((max, current) => 
      current[1].hours > max[1].hours ? current : max, 
      ['', { name: '', hours: 0 }]
    );
    const leastActiveProject = projectEntries.reduce((min, current) => 
      current[1].hours < min[1].hours ? current : min, 
      ['', { name: '', hours: Infinity }]
    );

    // Find top contributor
    const topContributor = Array.from(userMap.entries()).reduce((max, current) => 
      current[1].hours > max[1].hours ? current : max, 
      ['', { name: '', hours: 0 }]
    );

    // Calculate overall efficiency (project hours / total hours)
    const projectHours = data.filter(entry => entry.workType === 'project')
      .reduce((sum, entry) => sum + Number(entry.hours), 0);
    const overallEfficiency = totalHours > 0 ? (projectHours / totalHours) * 100 : 0;
    ...
    return {
      ...
      leastActiveProject: {
        ...
        totalHours: leastActiveProject[1].hours === Infinity ? 0 : leastActiveProject[1].hours,
      },
      topContributor: {
  ...
  async compareInsights(period1, period2, filters = {}) {
    ...
    return {
      ...
      changes: {
        ...
        efficiencyChange: metrics2.overallEfficiency - metrics1.overallEfficiency,
  ...
  async generateProjectInsight(projectId, data) {
    ...
    const projectHours = data.filter(entry => entry.workType === 'project')
      .reduce((sum, entry) => sum + Number(entry.hours), 0);
    const officeHours = data.filter(entry => entry.workType === 'office')
      .reduce((sum, entry) => sum + Number(entry.hours), 0);
    const otherHours = data.filter(entry => entry.workType === 'other')
      .reduce((sum, entry) => sum + Number(entry.hours), 0);

    const uniqueDays = new Set(data.map(entry => entry.date.toDateString())).size;
    const averageHoursPerDay = uniqueDays > 0 ? totalHours / uniqueDays : 0;

    // Work type distribution
    const workTypeDistribution = [
      { workType: 'project', hours: projectHours, percentage: (projectHours / totalHours) * 100, color: '#3b82f6' },
      { workType: 'office', hours: officeHours, percentage: (officeHours / totalHours) * 100, color: '#10b981' },
      { workType: 'other', hours: otherHours, percentage: (otherHours / totalHours) * 100, color: '#f59e0b' },
    ];
  ...
  buildSunburstData(data) {
    ...
    data.forEach(entry => {
      ...
      if (workType === 'project' && entry.projectName) {
        ...
        const existing = workTypeGroups.office.children.find(child => child.name === category);
        if (existing) {
        ...
        const category = entry.description || 'Other Internal Work';
        const existing = workTypeGroups.other.children.find(child => child.name === category);
        if (existing) {
          existing.value = (existing.value || 0) + Number(entry.hours);
        } else {
    ...
    const nonProjectChildren = [...workTypeGroups.office.children, ...workTypeGroups.other.children];
  ...
  calculateTopContributors(data) {
    ...
    userMap.forEach(contributor => {
      contributor.efficiency = contributor.totalHours > 0 ? (contributor.projectHours / contributor.totalHours) * 100 : 0;
      contributor.contributionPercentage = totalHours > 0 ? (contributor.totalHours / totalHours) * 100 : 0;
    });
