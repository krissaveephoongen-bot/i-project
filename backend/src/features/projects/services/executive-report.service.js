...
export class ExecutiveReportService {
  ...
  async getMonthlyReport(year) {
    ...
    const monthlyData = await this.db
      .select({
        ...
        totalEmployees: count(sql`DISTINCT ${timeEntries.userId}`).mapWith(Number),
        totalProjects: count(sql`DISTINCT ${timeEntries.projectId}`).mapWith(Number),
      })
      .from(timeEntries)
    ...
    return monthlyData.map(data => ({
      ...
      averageHoursPerPerson: data.totalEmployees > 0 ? (data.totalHours || 0) / data.totalEmployees : 0,
    }));
  }
  ...
  async getProjectReportSummary(filters = {}) {
    ...
    return projectData.map(data => ({
      ...
      totalHours: data.totalHours || 0,
      totalEmployees: data.totalEmployees || 0,
      averageHoursPerEmployee: data.totalEmployees > 0 ? (data.totalHours || 0) / data.totalEmployees : 0,
  ...
  async getEmployeeReportSummary(filters = {}) {
    ...
    const employeeData = await this.db
      .select({
        ...
        projectsWorked: count(sql`DISTINCT ${timeEntries.projectId}`).mapWith(Number),
      })
      .from(timeEntries)
      .leftJoin(users, eq(timeEntries.userId, users.id))
      .where(whereConditions.length > 0 ? and(...whereConditions) : undefined)
    ...
    return employeeData.map(data => ({
      ...
      efficiency: data.totalHours > 0 ? ((data.projectHours || 0) / data.totalHours) * 100 : 0,
  ...
  calculateSummary(data) {
    ...
    const totalProjects = new Set(data.filter(item => item.projectId).map(item => item.projectId)).size;
    const projectHours = data.filter(item => item.workType === 'project').reduce((sum, item) => sum + item.hours, 0);
    ...
    return {
      ...
      totalEmployees,
      totalProjects,
      averageHoursPerPerson: totalEmployees > 0 ? totalHours / totalEmployees : 0,
      projectHours,
      nonProjectHours,
