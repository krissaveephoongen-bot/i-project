 ...
 */
router.post('/requests', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 */
router.get('/requests/:id', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    ...
    // Check authorization
    if (request.userId !== req.user.id && req.user.role !== 'admin' && req.user.role !== 'manager') {
...
router.get('/requests', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (userId && userId !== req.user.id && req.user.role !== 'admin' && req.user.role !== 'manager') {
      throw new AppError(403, 'Not authorized to view other users leave requests');
    }
 ...
 */
router.put('/requests/:id', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
router.delete('/requests/:id', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.json({
      success: true,
      message: 'Leave request deleted successfully (implementation depends on your needs)',
    });
  } catch (error) {
    next(error);
  }
 ...
 */
router.post('/requests/:id/reject', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
 ...
 */
router.get('/for-approval', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const requests = await LeaveService.getLeaveRequestsForApproval(req.user.id);
 ...
 */
router.get('/allocations/:year', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (userId && userId !== req.user.id && req.user.role !== 'admin') {
      throw new AppError(403, 'Not authorized to view other users allocations');
    }

    const allocation = await LeaveService.getAllocation(targetUserId, parseInt(year));
 ...
 */
router.put('/allocations/:year', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { year } = req.params;
    ...
    const updated = await LeaveService.updateAllocation(userId, parseInt(year), {
      annualLeaveHours,
      usedLeaveHours,
    });
 ...
 */
router.get('/allocations/:year/balance', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { year } = req.params;
    const { userId } = req.query;
    ...
    const requests = await LeaveService.getUserLeaveRequests(targetUserId, 'pending');
