...
export const projects = pgTable('projects', {
  ...
  hourlyRate: numeric('hourlyRate', { precision: 10, scale: 2 }).default('0.00'),
...
export const tasks = pgTable('tasks', {
  ...
  actualHours: numeric('actualHours', { precision: 6, scale: 2 }).default('0.00'),
  weight: numeric('weight', { precision: 10, scale: 2 }).default('1.00'),
  completedAt: timestamp('completedAt'),
  projectId: uuid('projectId').references(() => projects.id, { onDelete: 'cascade' }).notNull(),
  assignedTo: uuid('assignedTo').references(() => users.id, { onDelete: 'set null' }),
  createdBy: uuid('createdBy').references(() => users.id, { onDelete: 'cascade' }).notNull(),
  parentTaskId: uuid('parentTaskId').references(() => tasks.id, { onDelete: 'cascade' }),
  category: text('category'),
  storyPoints: integer('storyPoints'),
  sprintId: uuid('sprintId'),
  blockedBy: uuid('blockedBy'),
