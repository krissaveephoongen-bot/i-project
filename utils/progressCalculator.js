const ProgressCalculator = {
  ...
  calculateProjectActualProgress: (tasks, worklogs) => {
    ...
    const totalWeight = tasks.reduce((sum, task) => sum + (task.objectData.Weight || 0), 0);
    
    if (totalWeight === 0) return 0;
    
    const weightedProgress = tasks.reduce((sum, task) => {
      const actualProgress = ProgressCalculator.calculateTaskActualProgress(task, worklogs);
  ...
  getTasksWithActualProgress: (tasks, worklogs) => {
    return tasks.map(task => ({
      ...task,
      actualProgress: ProgressCalculator.calculateTaskActualProgress(task, worklogs)
    }));
  }
};