...
export const DatabaseStatus: React.FC<DatabaseStatusProps> = ({
  ...
  const getStatusIcon = () => {
    switch (healthStatus.status) {
      ...
      case 'unhealthy': return 'alert-triangle';
  ...
  if (compact) {
    return (
      <div className="flex items-center space-x-2 p-2 rounded-lg bg-gray-50 border border-gray-200">
        <div className={`w-2 h-2 rounded-full ${getStatusColor()} ${healthStatus.status === 'loading' ? 'animate-pulse' : ''}`}></div>
        <span className="text-xs font-medium">
          {getStatusText()} - {healthStatus.message?.split(' ').slice(0, 3).join(' ')}...
        </span>
        <button
  ...
  return (
    <div className="database-status-component bg-white rounded-xl border border-gray-200 shadow-sm">
      <div className="flex items-center space-x-3 p-4">
        <div className={`w-4 h-4 rounded-full ${getStatusColor()} ${healthStatus.status === 'loading' ? 'animate-pulse' : ''} flex items-center justify-center`}>
        ...
        <div className="flex-1">
          <div className="font-medium text-sm text-gray-900">
            Database Status: <span className="font-semibold">{getStatusText()}</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <div className="font-medium text-green-700">{healthStatus.provider}</div>
              </div>
            </div>
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <div className="font-medium text-green-700">{healthStatus.database}</div>
              </div>
            </div>
            <div className="col-span-2 flex items-center space-x-2">
              ...
              <div>
                ...
                <div className="font-medium text-green-700 text-xs">{healthStatus.postgresVersion}</div>
              </div>
            </div>
            <div className="col-span-2 flex items-center space-x-2">
              ...
              <div>
                ...
                <div className="font-medium text-green-700 text-xs">{healthStatus.currentTime}</div>
              </div>
            </div>
      ...
      {showDetails && healthStatus.status !== 'healthy' && healthStatus.error && (
