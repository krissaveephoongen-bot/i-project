  ...
  try {
    ...
    return (
      <form onSubmit={handleSubmit} className="card max-w-2xl mx-auto">
        <h3 className="text-lg font-semibold mb-4">{member ? 'แก้ไขสมาชิก' : 'เพิ่มสมาชิกใหม่'}</h3>
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-2">อัตราต่อชั่วโมง (฿)</label>
            <input type="number" step="0.01" value={formData.hourlyRate} onChange={(e) => setFormData({...formData, hourlyRate: e.target.value})} className="w-full px-3 py-2 border rounded-lg" />
          </div>
          <div>
