  ...
  const tutorials = [
    {
      ...
      content: 'ระบบติดตามความคืบหน้าโครงการที่จะช่วยให้คุณจัดการงานได้อย่างมีประสิทธิภาพ',
      icon: 'hand-wave',
      color: 'blue'
    },
    {
    ...
    },
    {
      ...
      content: 'บันทึกเวลาทำงานประจำวัน ระบุโครงการและงานที่ทำ ระบบจะคำนวณ Manday อัตโนมัติ',
      icon: 'clock',
      color: 'teal'
    },
    {
      title: '4. ติดตามความคืบหน้า',
      content: 'ดูกราฟ S-Curve เพื่อเปรียบเทียบแผนกับผลงานจริง และติดตามงบประมาณ',
      icon: 'chart-bar',
      color: 'orange'
    },
    {
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden animate-fadeInUp relative">
        <div className="bg-gradient-to-r from-[var(--primary-color)] to-[var(--secondary-color)] p-6 text-white">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm opacity-80">ขั้นตอน {step + 1}/{tutorials.length}</span>
          </div>
          <h2 className="text-2xl font-bold">{current.title}</h2>
        </div>

        <div className="p-6">
          <p className="text-[var(--text-secondary)] text-lg mb-6">{current.content}</p>
          
          <div className="flex gap-2 mb-4">
            {tutorials.map((_, i) => (
              <div key={i} className={`h-2 flex-1 rounded-full ${i <= step ? 'bg-[var(--primary-color)]' : 'bg-gray-200'}`}></div>
            ))}
          </div>
