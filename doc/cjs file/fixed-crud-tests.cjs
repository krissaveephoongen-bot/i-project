...
/**
 * 🔧 Fixed Admin CRUD Testing Script
 * Resolves ES module issues and provides working CRUD tests
 */

const fs = require('fs');
const path = require('path');

console.log('🔧 Starting Fixed Admin CRUD Tests...\n');
...
class FixedCRUDTester {
  ...
  async testServerConnection() {
    ...
    try {
      // Simple HTTP request without external libraries
      const result = await this.makeSimpleRequest('GET', '/');
      ...
      if (result.success) {
        ...
        return { status: 'FAIL', error: 'Server connection failed' };
      }
    } catch (error) {
  ...
  makeSimpleRequest(method, path) {
    return new Promise((resolve) => {
      ...
      const req = http.request(options, (res) => {
        ...
        res.on('end', () => {
          resolve({ success: res.statusCode < 400, statusCode: res.statusCode, data });
        });
      });

      req.on('error', () => resolve({ success: false, error: 'Connection failed' }));
      req.on('timeout', () => resolve({ success: false, error: 'Request timeout' }));
      req.end();
    });
  }
  ...
  async testAPIEndpoint(endpoint, moduleName) {
    ...
    try {
      const result = await this.makeSimpleRequest('GET', endpoint);
      ...
      if (result.success) {
        ...
        console.log(`    ❌ ${moduleName} API failed: ${result.error || 'Unknown error'}`);
        return { 
          module: moduleName, 
          operation: 'API_ACCESS', 
          status: 'FAIL', 
  ...
  generateReport() {
    ...
    // Save JSON report
    fs.writeFileSync('fixed-test-results.json', JSON.stringify(report, null, 2));
  ...
  generateRecommendations() {
    ...
    const serverTest = this.results.find(r => r.module === 'Server Connection');
    if (serverTest && serverTest.status === 'FAIL') {
      recommendations.push('Start the Next.js development server: cd next-app && npm run dev');
    }

    const apiTests = this.results.filter(r => r.operation === 'API_ACCESS');
    const failedAPIs = apiTests.filter(r => r.status === 'FAIL');
    if (failedAPIs.length > 0) {
      recommendations.push('Check API routes configuration and ensure server is running');
    }

    const structTest = this.results.find(r => r.module === 'Project Structure');
    if (structTest && structTest.status === 'FAIL') {
      recommendations.push('Ensure all required CRUD files are present in the project');
    }

    const dbTest = this.results.find(r => r.module === 'Database');
    if (dbTest && dbTest.status === 'FAIL') {
      recommendations.push('Check database configuration and connection settings');
    }

    if (recommendations.length === 0) {
      recommendations.push('All tests passed! Your CRUD system is ready for testing.');
    }
...
<head>
    ...
    <title>Fixed CRUD Test Report</title>
    <style>
...
<body>
    <div class="container">
        ...
        <div class="modules">
            ${report.modules.map(module => `
                <div class="module">
                    ...
                    <div class="tests">
                        <div class="test">
                            <div class="test-name">${module.operation}</div>
                            <div class="test-status status-${module.status.toLowerCase()}">${module.status}</div>
                        </div>
                        ${module.error ? `<div style="color: #dc3545; font-size: 0.9em; margin-top: 10px;">Error: ${module.error}</div>` : ''}
                    </div>
                </div>
            `).join('')}
        </div>
        ...
        <div class="timestamp">
            Report generated by Fixed Admin CRUD Testing Suite
        </div>
    </div>
</body>
</html>`;

    fs.writeFileSync('fixed-test-report.html', html);
    console.log('✅ HTML report generated: fixed-test-report.html');
  }

  // Run all tests
  async runAllTests() {
    console.log('🚀 Starting Fixed CRUD Test Suite...\n');
    ...
    try {
      ...
      if (serverResult.status === 'PASS') {
        const apiResults = await Promise.all([
          this.testAPIEndpoint('/api/stakeholders', 'Stakeholders'),
          this.testAPIEndpoint('/api/expenses', 'Expenses'),
          this.testAPIEndpoint('/api/admin/users', 'Admin Users'),
          this.testAPIEndpoint('/api/timesheet', 'Timesheet')
        ]);
      ...
      console.log('\n📄 Reports generated:');
      console.log('   - fixed-test-results.json (detailed data)');
      console.log('   - fixed-test-report.html (visual report)');
...
if (require.main === module) {
  const tester = new FixedCRUDTester();
  tester.runAllTests().catch(console.error);
}

module.exports = FixedCRUDTester;
