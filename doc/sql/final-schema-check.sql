-- ==========================================
-- Final Schema Check with Correct Column Names
-- ตรวจสอบสถานะปัจจุบันของ database schema (แก้ไข column names)
-- ==========================================
...
-- ==========================================
-- 8. CHECK CURRENT DATA STATUS (Fixed Column Names)
-- ==========================================
...
-- Tasks data status (Fixed: assigned_to instead of assignee_id)
SELECT 
    'Tasks' as table_name,
    COUNT(*) as total_records,
...
-- ==========================================
-- 10. SAMPLE DATA VERIFICATION (Fixed Column Names)
-- ==========================================
...
-- Sample tasks (Fixed: assigned_to instead of assignee_id)
SELECT 
...
-- ==========================================
-- 11. CORRECTED REQUIRED COLUMNS CHECK
-- ==========================================

-- Check if required columns exist (Fixed column names)
SELECT 
...
WHERE table_schema = 'public'
    ...
    AND column_name IN ('code', 'category', 'manager_id', 'client_id', 'weekly_capacity', 'project_id', 'assigned_to', 'status', 'priority')
ORDER BY table_name, column_name;

-- Check missing columns (Fixed column names)
SELECT 
...
-- ==========================================
-- 12. COLUMN NAME MAPPING CHECK
-- ==========================================

-- Show actual column names vs expected names
SELECT 
    'Column Name Mapping' as check_type,
    table_name,
    column_name as actual_name,
    CASE 
        WHEN column_name = 'assigned_to' THEN 'assignee_id (expected)'
        WHEN column_name = 'assignee_id' THEN 'assignee_id (correct)'
        ELSE column_name
...
-- 5. Indexes ทั้งหมด
-- 6. สถานะข้อมูลปัจจุบัน (แก้ไข column names แล้ว)
-- 7. ปัญหา duplicate codes
-- 8. ข้อมูลตัวอย่างจากแต่ละตาราง (แก้ไข column names แล้ว)
-- 9. ตรวจสอบคอลัมน์ที่จำเป็น (แก้ไข column names แล้ว)
-- 10. ตรวจสอบคอลัมน์ที่หายไป (แก้ไข column names แล้ว)
-- 11. ตรวจสอบการ mapping ของ column names
-- ==========================================
