-- ==========================================
-- Fixed Comprehensive Database Setup for Project Management System
...
-- ==========================================
-- 3. FOREIGN KEY CONSTRAINTS & RELATIONS (Fixed)
-- ==========================================
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_projects_manager_id' AND table_name = 'projects') THEN
        ALTER TABLE projects DROP CONSTRAINT fk_projects_manager_id;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_projects_client_id' AND table_name = 'projects') THEN
        ALTER TABLE projects DROP CONSTRAINT fk_projects_client_id;
    END IF;
    
    -- Tasks table relations
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_tasks_project_id' AND table_name = 'tasks') THEN
        ALTER TABLE tasks DROP CONSTRAINT fk_tasks_project_id;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_tasks_assignee_id' AND table_name = 'tasks') THEN
        ALTER TABLE tasks DROP CONSTRAINT fk_tasks_assignee_id;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_tasks_creator_id' AND table_name = 'tasks') THEN
        ALTER TABLE tasks DROP CONSTRAINT fk_tasks_creator_id;
    END IF;
    
    -- Milestones table relations
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_milestones_project_id' AND table_name = 'milestones') THEN
        ALTER TABLE milestones DROP CONSTRAINT fk_milestones_project_id;
    END IF;
    
    -- Time entries table relations
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_time_entries_user_id' AND table_name = 'time_entries') THEN
        ALTER TABLE time_entries DROP CONSTRAINT fk_time_entries_user_id;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_time_entries_task_id' AND table_name = 'time_entries') THEN
        ALTER TABLE time_entries DROP CONSTRAINT fk_time_entries_task_id;
    END IF;
    
    -- Expenses table relations
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_expenses_user_id' AND table_name = 'expenses') THEN
        ALTER TABLE expenses DROP CONSTRAINT fk_expenses_user_id;
    END IF;
    
    -- Timesheets table relations
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_timesheets_user_id' AND table_name = 'timesheets') THEN
        ALTER TABLE timesheets DROP CONSTRAINT fk_timesheets_user_id;
    END IF;
END $$;

-- Now add the constraints (without IF NOT EXISTS)
ALTER TABLE projects 
...
ADD CONSTRAINT fk_timesheets_user_id 
FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;

-- ==========================================
-- 4. UNIQUE CONSTRAINTS (Fixed)
-- ==========================================

-- Drop unique constraints first if they exist
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'projects_code_unique' AND table_name = 'projects') THEN
        ALTER TABLE projects DROP CONSTRAINT projects_code_unique;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'users_email_unique' AND table_name = 'users') THEN
        ALTER TABLE users DROP CONSTRAINT users_email_unique;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'tasks_project_name_unique' AND table_name = 'tasks') THEN
        ALTER TABLE tasks DROP CONSTRAINT tasks_project_name_unique;
    END IF;
END $$;
