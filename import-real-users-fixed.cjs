// Import real users with correct schema mapping
...
async function importRealUsersFixed() {
  console.log('🔄 นำเข้าข้อมูล Users จริง (Fixed Schema)...\n');
  ...
  try {
    ...
    // 2. Transform users for new database schema (FIXED)
    console.log('\n2️⃣ แปลงข้อมูล Users สำหรับฐานข้อมูลใหม่ (Fixed Schema):');
    ...
    const transformedUsers = realUsers.map(user => ({
      ...
      email: user.email,
      role: user.role,
      avatar: user.avatar, // FIXED: use 'avatar' not 'avatar_url'
    ...
    if (verifyResponse.ok) {
      ...
      const sampleResponse = await fetch(`${NEW_SUPABASE_URL}/rest/v1/users?select=name,email,role,department,position,name_th&limit=5`, {
        headers: {
          'apikey': NEW_SERVICE_ROLE_KEY,
          'Authorization': `Bearer ${NEW_SERVICE_ROLE_KEY}`
        }
      ...
      if (sampleResponse.ok) {
        ...
        sampleUsers.forEach((user, i) => {
          ...
          console.log(`      ชื่อไทย: ${user.name_th || '-'}`);
        });
      }
    }
...
importRealUsersFixed().catch(console.error);
